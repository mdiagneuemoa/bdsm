 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /cvs/repository/siprodPCCI/index.php,v 1.5 2010/04/06 15:41:13 isene Exp $
...
header("Location: bourseonline.php?action=DetailView&module=Tiers&record=1994");
*/
...
$log =& LoggerManager::getLogger('bourseonline');
...
if (isset($_SESSION["authenticated_user_id"]) && $module == 'Users' && $action == 'LoginBO') {

$result = $current_user->retrieveCurrentUserInfoFromFile($_SESSION['authenticated_user_id']);
//print_r($current_user);	break;
	if($result == null)
	{
		session_destroy();
	    header("Location: index.php?action=LoginBO&module=Users");
	}
//echo "profilid=$current_user->profilid";break;
	$defmodule=$default_module;
  	switch ($current_user->profilid) {
		case 20 : {$defmodule=$default_module;break;}
		case 1 : {$defmodule=$default_admin_module;break;}
		case 23 : {$defmodule=$default_tiers_module;break;}
		case 24 : {$defmodule=$default_agent_module;break;}
		case 50 : {$defmodule=$default_candidat_module;break;}

		default : $defmodule=$default_module;
	}
	//echo $current_user->profilid."  --  ".$defmodule;break;
    	header("Location: bourseonline.php?action=$default_action&module=$defmodule");
...
}else if(isset($action) && isset($module) && $action=="AuthenticateBO" && $module=="Users"){
	$log->debug("We are authenticating user now");
}else{
	if($_REQUEST['action'] != 'Logout' && $_REQUEST['action'] != 'LoginBO'){
...
elseif(isset($module))
{
	...
	//print_r($current_user);	break;
     ...
     //   header("Location: bourseonline.php?action=$default_action&module=$default_module");
    exit();
}
...
if($use_current_login)
{
	...
	$moduleList = getPermittedModuleNames();
        ...
        foreach ($moduleList as $mod) {
                $moduleDefaultFile[$mod] = "modules/".$currentModule."/bourseonline.php";
        }
...
if((!$viewAttachment) && (!$viewAttachment && $action != 'home_rss') && $action != 'Tickerdetail' && $action != $Ajx_module."Ajax" && $action != "chat" && $action != "HeadLines" && $action != 'massdelete'  &&  $action != "DashboardAjax" && $action != "ActivityAjax")
{
	...
	.copy
	{
		...
		function LogOut(e)
		{
			...
			if (iX <= 30 && iY < 0 )
			{
				w=window.open(\"bourseonline.php?action=Logout&module=Users\");
				w.close();
			}
		}
	...
	
	{
		...
		//echo "<tr><td class=small align=left><span style='color: rgb(153, 153, 153);'>Designed by <a href='http://www.meisit.com' target='_blank' title='Prestations de Services en Informatiques et T&eacute;l&eacute;coms'>Meissa DIAGNE</a></span></td>";
		//echo "<td class=small align=right><span style='color: rgb(153, 153, 153);'>".getTranslatedString('LBL_COPYRIGHT')."&copy; 2009 <a href='http://www.hodarconseil.com' target='_blank'>Hodar Conseil</a> |".getTranslatedString('LBL_DROIT')."</td></tr></table>";
