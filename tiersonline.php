...
header("Location: tiersonline.php?action=DetailView&module=Tiers&record=1994");
*/
...
if (isset($_SESSION["authenticated_user_id"]) && $module == 'Users' && $action == 'LoginTiers') {
...
//print_r($current_user);	break;
	...
	{
		session_destroy();
	    header("Location: tiersonline.php?action=LoginTiers&module=Users");
	}
//echo "profilid=$current_user->profilid";break;
	...
	//echo $current_user->profilid."  --  ".$defmodule;break;
    	header("Location: tiersonline.php?action=$default_action&module=$defmodule");
...
//		$_SESSION['internal_mailer'] = $adb->query_result($qry_res,0,"internal_mailer");
	...
	if($_REQUEST['action'] != 'Logout' && $_REQUEST['action'] != 'LoginTiers'){
...
elseif(isset($module))
{
	...
	//print_r($current_user);	break;
     ...
     //   header("Location: tiersonline.php?action=$default_action&module=$default_module");
    exit();
}
...
if((!$viewAttachment) && (!$viewAttachment && $action != 'home_rss') && $action != 'Tickerdetail' && $action != $Ajx_module."Ajax" && $action != "chat" && $action != "HeadLines" && $action != 'massdelete'  &&  $action != "DashboardAjax" && $action != "ActivityAjax")
{
	...
	.copy
	{
		...
		function LogOut(e)
		{
			...
			if (iX <= 30 && iY < 0 )
			{
				w=window.open(\"tiersonline.php?action=Logout&module=Users\");
				w.close();
			}
		}
