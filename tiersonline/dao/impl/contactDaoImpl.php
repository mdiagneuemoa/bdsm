<?php
require_once (PATH_DAO_INTERF."/IContactDao.php");  
 ...
 {   
    ...
    public function updateChampContact($id,$champ,$valeur)
    {
		...
		if (!$db->Query($sql)) {
			...
			$trace->Trace_Err("requete = [$sql], Contact =[" . $db->Errno . "] ,code = [" . $db->Error . "]");
		}
    }
    ...
    public function selectContactByAbonne($numeroclient)
	{
		global $trace;
    	$db = Mysql::getInstance(DB_HOST, DB_DATABASE, DB_USER, DB_PASSWORD);
		$db->Open();
