...
function activeUser(obj,userid,user)
{
		if(confirm("Etes-vous sure de vouloir activer cet utilisateur ?"))
		{
	        $("status").style.display="inline";
	        new Ajax.Request(
	                \'index.php\',
	                {queue: {position: \'end\', scope: \'command\'},
	                        method: \'post\',
	                        postBody: \'action=UsersAjax&file=ActiveUser&return_action=ListView&return_module=Users&module=Users&parenttab=Settings&record=\'+userid,
	                        onComplete: function(response) {
									 $("status").style.display="none";
	                                $("st_"+userid).innerHTML= response.responseText;
					        }
	                }
	        );
		}	
