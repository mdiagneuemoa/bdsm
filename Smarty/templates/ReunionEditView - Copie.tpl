									...
									<table border=0 cellspacing=0 cellpadding=0 width=100% class="small">
									   ...
									   {foreach key=header item=data from=$BLOCKS}
										...
										{if $header== $MOD.LBL_REUNION_DEPENSES && $MODULE == 'Reunion'}
													...
													<td colspan=6 class="detailedViewHeader">
														<table width=99% border=0>
															<tr>
																...
																<td align=right>
																	{if $BUDGETDISP.$CODEBUDGET.$comptenat.mntdispo neq ''}
																		<b>Montant disponible : {$BUDGETDISP.$CODEBUDGET.$comptenat.mntdispo} FCFA</b>
																	{else}
												...
												<tr>
													<td colspan=6>
														<table width=95% border=1 id="naturesdepense_{$comptenat}">
															{foreach item=lignedepense name=linedep from=$natdepense.depenses}
																<tr>
																	<td ><input type="text" name="naturesdepense_{$comptenat}_lib_{$smarty.foreach.linedep.index+1}" id="naturesdepense_{$comptenat}_lib_{$smarty.foreach.linedep.index}" value="{$lignedepense.libdepense}" size=70></td>
																	<td align="center"><input type="text" name="naturesdepense_{$comptenat}_qte_{$smarty.foreach.linedep.index+1}" id="naturesdepense_{$comptenat}_qte_{$smarty.foreach.linedep.index}" onmouseout="calculTotalDepense('{$comptenat}','{$smarty.foreach.linedep.index}')" value="{$lignedepense.qtedepense}" size=10></td>
																	<td align="center"><input type="text" name="naturesdepense_{$comptenat}_nb_{$smarty.foreach.linedep.index+1}" id="naturesdepense_{$comptenat}_nb_{$smarty.foreach.linedep.index}" onmouseout="calculTotalDepense('{$comptenat}','{$smarty.foreach.linedep.index}')" value="{$lignedepense.nbredepense}" size=10></td>
																	<td align=right><input type="text" name="naturesdepense_{$comptenat}_pu_{$smarty.foreach.linedep.index+1}" id="naturesdepense_{$comptenat}_pu_{$smarty.foreach.linedep.index}" onmouseout="calculTotalDepense('{$comptenat}','{$smarty.foreach.linedep.index}')" value="{$lignedepense.pudepense}"  size=20></td>
																	<td align=right><input type="text" name="naturesdepense_{$comptenat}_pt_{$smarty.foreach.linedep.index+1}" id="naturesdepense_{$comptenat}_pt_{$smarty.foreach.linedep.index}" value="{$lignedepense.totaldepense}" style="text-align: right;" size=20></td>
																	<td nowrap>
																	{if $smarty.foreach.linedep.index == 0}
																	...
																	{else}
																		<a href="javascript:;" onClick="supLigneDepenseElt('{$comptenat}','{$smarty.foreach.linedep.index+1}');"><img src="modules/Reunion/rowdelete.gif" titre="Suprimer cette ligne de dépense"></a>
																	{/if}
																	</td>
																</tr>
											...
											<td align=right><b>{$totaldepreunion|number_format:0:",":" "}</b></td>
											<td align="center">&nbsp;</td>
											</tr>
											</table>
												</td>
