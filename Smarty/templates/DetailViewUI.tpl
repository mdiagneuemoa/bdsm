...
<!-- This file is used to display the fields based on the ui type in detailview -->
		...
		{if $keyid eq '1' || $keyid eq 2 || $keyid eq '11' || $keyid eq '7' || $keyid eq '9' || $keyid eq '55' || $keyid eq '71' || $keyid eq '72' || $keyid eq '103' || $keyid eq '255' || $keyid eq '300'} <!--TextBox-->
			<td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}','{$keyfldname}');" onmouseout="fnhide('crmspanid');" valign="top">
				...
				{if $keyid eq 11}
					...
					{/if}
				{/if}
				{if $keyid eq 1 && $keyfldname eq 'folderid'}	
					&nbsp;<img src="{'dossier-ferme.gif'|@vtiger_imageurl:$THEME}">&nbsp;<span id="dtlview_{$label}">{$FOLDERNAME}</span>
					
				
					
				{elseif $keyfldname eq 'nom' && ($MODULE eq 'Demandes') }
				...
				{elseif $keyfldname eq 'commentbillet' && $MODULE eq 'OrdresMission'}
					...
					<span id="dtlview_{$label}">{$AGENTNOM}</span>	
					{else}
					{if $MODULE eq 'Incidents' && $keyfldname eq 'popimpactee'}
						&nbsp;&nbsp;<span id="dtlview_{$label}">{$keyval}{$ALL_POPULATION_CHECKED}</span>
					{else}
						&nbsp;&nbsp;<span id="dtlview_{$label}">{$keyval}</span>
					{/if}
				{/if}
        ...
        {elseif $keyid eq '13' || $keyid eq '104'} <!--Email-->
            <td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"><span id="dtlview_{$label}">
 			...
 			{*if $keyfldname eq 'statut' && ($CATEGORY eq 'Incidents' || $CATEGORY eq 'Demandes')}
				<td class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value="posteur"></input>{$MOD.POSTEUR} </td>
				<td class="dvtCellInfo" align="left" width=25%">&nbsp;{$POSTEUR_INFO}</td>
			{/if*}
	  ...
	  {elseif $keyid eq '33'}<!--Multi Select Combo box-->
        ...
        {elseif $keyid eq '19' || $keyid eq '20'} <!--TextArea/Description-->
				...
				{/if}
				{if $MODULE eq 'Documents' || $MODULE eq 'HReports' || $MODULE eq 'Demandes' || $MODULE eq 'Incidents'}
					{if $keyfldname eq 'description' || $keyfldname eq 'objet' || $keyfldname eq 'commentbillet'}
		...
		{elseif $keyid eq 5}
			...
			{if $keyfldname eq 'datelivraison' && $CATEGORY eq 'Demandes'}
				<td class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value="posteur"></input>{$MOD.POSTEUR} </td>
				<td class="dvtCellInfo" align="left" width=25%">&nbsp;{$POSTEUR_INFO}</td>
			{/if}
		...
		{else}<!-- for Image Reflection and : Hodar crm  $keyid neq '27' pour ne pas afficher filelocationtype -->
			<td class="dvtCellInfo" align="left" width=25%">&nbsp;{$keyval}</td>
			
			{*if $keyfldname eq 'createdtime' && $CATEGORY eq 'Incidents'}
				<td class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value="posteur"></input>{$MOD.POSTEUR} </td>
				<td class="dvtCellInfo" align="left" width=25%">&nbsp;{$POSTEUR_INFO}</td>
			{/if*}
			{if $keyfldname eq 'heurefin_relle' && $CATEGORY eq 'Incidents'}
				<tr style="height:25px"><td class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value="posteur"></input>{$MOD.POSTEUR} </td>
				<td class="dvtCellInfo" align="left" width=25%">&nbsp;{$POSTEUR_INFO}</td></tr>
			{/if}
