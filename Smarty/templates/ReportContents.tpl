...
<table align="center" border="0" cellpadding="0" cellspacing="0" width="99%"  class="showPanelBg">
	...
	<td valign="top" width="70%" style="padding: 10px;border-right:1px dashed #CCCCCC">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" class="reportsListTable">
		...
		<tr>
		<td>
		<table  class="lvt small" border="0" cellpadding="5" cellspacing="1" width="100%">
			<tbody>
			<tr>
			...
			{foreach name=reportdtls item=reportdetails from=$reportfolder.details}
				...
				{if $MOD[$reportdetails.reportname] neq ''}
					<td>
						<span class="fontMarron">
							<a href="index.php?module=Reports&action=SaveAndRun&record={$reportdetails.reportid}&folderid={$reportfolder.id}">{$MOD[$reportdetails.reportname]}</a></td>
						</span>
				{else}  
					<td>
						<span class="fontMarron">
							<a href="index.php?module=Reports&action=SaveAndRun&record={$reportdetails.reportid}&folderid={$reportfolder.id}">{$reportdetails.reportname}</a></td>
						</span>	
				{/if}
