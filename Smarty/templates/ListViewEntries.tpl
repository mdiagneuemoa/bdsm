...
<form name="massdelete" method="POST" id="massdelete">
     ...
     <input name="current_page_boxes" id="current_page_boxes" type="hidden" value="{$CURRENT_PAGE_BOXES}">
				...
				-->
				{if  ($IS_DIRCABPCOM eq '1') && ($SEARCH_FIELD_VAL eq 'umv_accepted')} 
					<td style="padding-right:20px" nowrap>
						<input 
				...
				{if  ($IS_DIRCABPCOM eq '1') && ($SEARCH_FIELD_VAL eq 'dcpc_submitted')} 
					<td style="padding-right:20px" nowrap>
						<input 
							title="{$MOD.LBL_REJET_DCPC_BUTTON_LABEL}" 
							class="crmbutton small edit" 
				...
				<td width=100% align="right">
				   ...
				   {if $HIDE_CUSTOM_LINKS neq '1'}
					<table border=0 cellspacing=0 cellpadding=0 class="small">
					<tr>
						{*if $NB_RECORDS neq 0 && $NB_RECORDS neq ''}
		   ...
		   <td class="lvtCol"><input type="checkbox"  name="selectall" onClick=toggleSelect_ListView(this.checked,"selected_id")></td>
			...
			{foreach name="listviewforeach" item=header from=$LISTHEADER}
				{if $header eq "Action" && ($MODULE eq "Conventions" || $MODULE eq "SBConventions" || $MODULE eq "Demandes" )}
				{else}
 					<td class="lvtCol" align=center nowrap>{$header}</td>
 				{/if}
			{/foreach}
			</tr>
			<!-- Table Contents -->
			 {if ($MODULE eq 'ReportingRegie' || $MODULE eq 'ReportingUMV') && $NB_RECORDS neq 0}
				{assign var=totaldecomptes value=0}
				{assign var=totaljours value=0}
				{assign var=nbmissions value=0}
			
			{/if}
			{if ($MODULE eq 'ReportingUMV') && $NB_RECORDS neq 0}
					<tr bgcolor=white class=lvtColDataHover  id="row_{$entity_id}">
						<td colspan="3" align="right"><b>Cumul Global</b></td>
						<td align="right"><b>{$TOTALDUREE}</td>
						<td align="right"><b>{$TOTALMISSION}</td>
						<td align="right"><b>{$TOTALMONTANT|number_format:0:",":" "}</td>
			...
			{foreach item=data name=cols from=$entity}
					...
					{if $MODULE eq 'Conventions' && $smarty.foreach.cols.index == 3}
					...
					{elseif $MODULE eq 'ReportingRegie' && $smarty.foreach.cols.index == 6}
					{assign var=totaljours value=$totaljours+$data}
						<td nowrap align="right">
							{$data}
						</td>
					{elseif $MODULE eq 'ReportingUMV' && $smarty.foreach.cols.index == 2}
					{assign var=totaljours value=$totaljours+$data}
						<td nowrap align="right">
							{$data}
						</td>
					{elseif $MODULE eq 'ReportingUMV' && $smarty.foreach.cols.index == 3}
					{assign var=nbmissions value=$nbmissions+$data}
						<td nowrap align="right">
							{$data}
						</td>
					{elseif $MODULE eq 'ReportingUMV' && $smarty.foreach.cols.index == 4}
					{assign var=totaldecomptes value=$totaldecomptes+$data}
						<td nowrap align="right">
							{php} 
								$mnt = $this->get_template_vars('data'); 
								$mnt2 = number_format($mnt, 0, ',', ' ');
					...
					{elseif $MODULE eq 'ReportingRegie' && $smarty.foreach.cols.index == 7}
					{assign var=totaldecomptes value=$totaldecomptes+$data}
						<td nowrap align="right">
							{php} 
								$mnt = $this->get_template_vars('data'); 
			...
			{else}
				...
				{/if}
			     ...
			     {/foreach}
				  {if ($MODULE eq 'ReportingRegie') && $NB_RECORDS neq 0}
					<tr bgcolor=white class=lvtColDataHover  id="row_{$entity_id}">
			 ...
			 <table border=0 cellspacing=0 cellpadding=2 width=100%>
				 ...
				 <td style="padding-right:20px" nowrap>
					...
					-->
				 <td style="padding-right:20px" class="small" nowrap width=40%>{$RECORD_COUNTS}</td>
