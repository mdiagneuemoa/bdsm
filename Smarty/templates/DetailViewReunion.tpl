...
<tr>
	...
	<td class="showPanelBg" valign=top width=100%>
			...
			<td>
				...
				<tr>
					...
					{if ($IS_POSTEURDEMANDE eq '1' || $CURRENT_USER_PROFIL eq '20') && ($STATUT neq 'mt_cancelled' || $STATUT neq 'cancelled') }
						...
						<input 
										title="{$MOD.LBL_CANCELREUNION_BUTTON_LABEL}" 
										...
										type="submit" 
										name="Edit" 
										value="&nbsp;{$MOD.LBL_CANCELREUNION_BUTTON_LABEL}&nbsp;">&nbsp;
							
							
						<input title="{$MOD.LBL_DOCREUNION_BUTTON_LABEL}" class="crmbutton small edit" 
										onclick="window.open('http://ouavlibre01/portailnomade/reunions/reunion.php?reunionid={$ID}','','');return false;"   
										type="submit"  
										name="Edit" 
										value="&nbsp;{$MOD.LBL_DOCREUNION_BUTTON_LABEL}&nbsp;">&nbsp;
						...
						{/if}
						<input 
							title="{$MOD.LBL_ADD_BUDGETCOMPLEMENTAIRE_BUTTON_LABEL}" 
							class="crmbutton small edit" 
						...
						type="submit"  
						name="Edit"
							value="&nbsp;{$MOD.LBL_ADD_BUDGETCOMPLEMENTAIRE_BUTTON_LABEL}&nbsp;">&nbsp;
						{if $IS_DEMANDEVALIDE eq '1'}
							...
							{elseif $STATUT eq 'db_accepted'}
								<input title="{$MOD.LBL_ADD_SIGNATAIRE_BUTTON_LABEL}" class="crmbutton small edit" 
								onclick="AddSignataire();"   
								type="button"  
								name="Edit" 
								value="&nbsp;{$MOD.LBL_ADD_SIGNATAIRE_BUTTON_LABEL}&nbsp;">&nbsp;	
					...
					{if  ($IS_AGENTDB eq '1'  || $CURRENT_USER_PROFIL eq '20')} {*    PROFIL AGENT DB *}
								...
								{if $STATUT eq 'dc_authorized' || $STATUT eq 'db_accepted'}
									<input 
										title="{$MOD.LBL_CREATEDECISION_BUTTON_LABEL}" 
										class="crmbutton small edit" 
										onclick="return SaisirDecision({$ID});"   
										type="button"  
										name="Edit" 
										value="&nbsp;{$MOD.LBL_CREATEDECISION_BUTTON_LABEL}&nbsp;">&nbsp;
								{/if}	
		...
		{if $MODULE == 'Reunion' }
			{if $STATUT eq 'open'}
				<tr><td align="left" ><img src="{'open_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
				
			{if $STATUT eq 'submitted'}
				<tr><td align="left" ><img src="{'submitted_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>				
			{/if}	
			{if $STATUT eq 'dc_accepted'}
				<tr><td align="left" ><img src="{'dc_accepted_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			{if $STATUT eq 'dc_cancelled' || $STATUT eq 'dc_denied'}
				<tr><td align="left" ><img src="{'dc_denied_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			{if $STATUT eq 'db_accepted'}
				<tr><td align="left" ><img src="{'db_accepted_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			
			{if $STATUT eq 'db_denied'}
				<tr><td align="left" ><img src="{'db_denied_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			
			{if $STATUT eq 'dcf_accepted'}
				<tr><td align="left" ><img src="{'dcf_accepted_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			{if $STATUT eq 'dcf_denied'}
				<tr><td align="left" ><img src="{'dcf_denied_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			
			{if $STATUT eq 'com_cancelled' || $STATUT eq 'com_denied'}
				<tr><td align="left" ><img src="{'com_denied_reunion.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
						...
						<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="traitementbut" onclick="this.form.action.value='SaveTraitement';" type="submit" " name="button" value="Enregistrer Livraison" >
							...
							{if $header== $MOD.LBL_REUNION_DEPENSES && $MODULE == 'Reunion'}
									...
									{foreach item=natdepense key=comptenat from=$NATDEPENSES}
											...
											<td colspan=5 class="detailedViewHeader">
												<table width=99% border=0>
													<tr>
														...
														<td align=right>
															{if $BUDGETDISP.$CODEBUDGET.$comptenat.mntdispo neq '' && $BUDGETDISP.$CODEBUDGET.$comptenat.mntdispo neq '0'}
																<b>Montant disponible : <font color='green'>{$BUDGETDISP.$CODEBUDGET.$comptenat.mntdispo|number_format:0:",":" "} FCFA </font></b>
															{else}
										...
										{foreach item=lignedepense from=$natdepense.depenses}
											<tr>
										...
										{/foreach}
											<tr>
												...
												<td align=right>
												{if $natdepense.totaldepense gt $BUDGETDISP.$CODEBUDGET.$comptenat.mntdispo}
													<font color='red'><b>{$natdepense.totaldepense|number_format:0:",":" "}</b></font>
												{else}
									...
									<td align=right><b><font color='blue'>{$totaldepreunion|number_format:0:",":" "} FCFA</font></b></td></tr>
									</table>
										</td>
		...
		<div id='divdecision'  style='display:none;'>
			...
			{if $MODULE eq 'Reunion'}
					<tr><td>&nbsp;</td></tr>
					<tr><td class="small"><b><u>Inititeur R&eacute;union</u> : {$POSTEURDEMANDE}<b></td></tr>	
				{/if}										
		<tr>           
				{* ************************************* NOMADE ************************************ *}
		...
		<!-- Inventory - Product Details informations -->
		   <tr>
			...
			-->
 				...
 				{include file='ListViewTraitementReunions.tpl'}
