...
<tr>
	...
	<td class="showPanelBg" valign=top width=100%>
				...
				<tr>
					...
					{if  $MODULE eq 'Demandes'}
						...
						{if ($IS_POSTEURDEMANDE eq '1' || $CURRENT_USER_PROFIL eq '20') && ($STATUT neq 'om_cancelled' || $STATUT neq 'ag_cancelled' || $STATUT neq 'ch_cancelled' || $STATUT neq 'ch_cancelled') }
							...
							{if $STATUT eq 'dcpc_submitted' || $STATUT eq 'dc_submitted' || $STATUT eq 'dc_denied' || $STATUT eq 'dc_authorized'}
							...
							{if $STATUT eq 'open'}
								...
								{else}
									...
									<input 
										title="{$MOD.LBL_DC_SOUMETTRE_BUTTON_LABEL}" 
										class="crmbutton small edit" 
										onclick="this.form.return_module.value='Demandes'; 
											this.form.return_action.value='DetailView'; 
											this.form.module.value='TraitementDemandes';
									...
									type="submit"  
									name="Edit" 
									value="&nbsp;{$MOD.LBL_DC_SOUMETTRE_BUTTON_LABEL}&nbsp;">&nbsp;
							{/if}				
					...
					{if  ($CURRENT_USER_PROFIL eq '22' || $IS_INTERIMDIRCAB eq '1') && ($IS_DIRCABPCOM neq '1') && $STATUT eq 'dc_submitted'} {*    PROFIL DIRCAB *}
						<input 
					...
					{if ($IS_DIRCABPCOM eq '1' || $IS_INTERIMDIRCABPCOM eq '1') && $STATUT eq 'dcpc_submitted'} {*    PROFIL DIRCAB PCOM *}
						<input 
										title="{$MOD.LBL_REJET_DC_BUTTON_LABEL}" 
										class="crmbutton small edit" 
										onclick="this.form.return_module.value='Demandes'; 
					...
					{if  ($CURRENT_USER_PROFIL eq '26'  || $CURRENT_USER_PROFIL eq '20' || $IS_INTERIMRUMV eq '1') && $STATUT neq 'om_cancelled'}  {*    PROFIL RESPONS UMV *}
							...
							{if  $STATUT eq 'dc_authorized' || $STATUT eq 'authorized'}
								<input 
					...
					{/if}
					{if  ($CURRENT_USER_PROFIL eq '20') && $STATUT eq 'omgenered'}	{*    PROFIL AGENT UMV *}
								<input title="{$APP.LBL_EDIT_BUTTON_TITLE}" accessKey="{$APP.LBL_EDIT_BUTTON_KEY}" class="crmbutton small edit" onclick="this.form.return_module.value='{$MODULE}'; this.form.return_action.value='DetailView'; this.form.return_id.value='{$ID}';this.form.module.value='{$MODULE}';this.form.action.value='EditView'" type="submit" name="Edit" value="&nbsp;{$APP.LBL_EDIT_BUTTON_LABEL}&nbsp;">&nbsp;
					{/if}							
		...
		{if $MODULE == 'Demandes' }
			{if $STATUT eq 'open'}
				<tr><td align="left" ><img src="{'open_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}	
			{if $STATUT eq 'ag_cancelled' || $STATUT eq 'ch_cancelled'}
				<tr><td align="left" ><img src="{'cancelled_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			{if $STATUT eq 'dc_submitted' || $STATUT eq 'dcpc_submitted' }
				<tr><td align="left" ><img src="{'dc_submitted_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
				
			{if $STATUT eq 'dc_authorized'}
				<tr><td align="left" ><img src="{'dc_authorized_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			
			{if $STATUT eq 'dc_cancelled' || $STATUT eq 'dc_denied'}
				<tr><td align="left" ><img src="{'dc_denied_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
						
			{if $STATUT eq 'umv_denied' }
				<tr><td align="left" ><img src="{'umv_denied_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			
			{if $STATUT eq 'rumv_accepted' || $STATUT eq 'umv_accepted'}
				<tr><td align="left" ><img src="{'umv_accepted_agdepart.png'|@vtiger_imageurl:$THEME}"></td></tr>
			{/if}
			{/if}
