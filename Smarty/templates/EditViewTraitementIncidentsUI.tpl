...
-->*} 
		...
		{else}
			{assign var="mandatory_field" value="(f)"}
		{/if}
...
alt="Clear" title="Clear" LANGUAGE=javascript	onClick="this.form.{$fldname}.value=''; this.form.{$fldname}_display.value=''; return false;" align="absmiddle" style='cursor:hand;cursor:pointer'>&nbsp;
		...
		{elseif $uitype eq 2}
			...
			{if $fldname eq 'ticket' && $MODULE eq 'Incidents' && $OP_MODE eq 'create_view' }
				<td width=30% align=left class="dvtCellInfo">
			...
			{elseif $fldname eq 'user' && $MODULE eq 'TraitementIncidents' && $OP_MODE eq 'create_view' }
				<td width=30% align=left class="dvtCellInfo">
			...
			{elseif $fldname eq 'user' && $MODULE eq 'TraitementIncidents' && $OP_MODE eq 'edit_view' }
				<td width=30% align=left class="dvtCellInfo">
					<input type="text" name="vue" tabindex="{$vt_tab}" readonly="true" value="{$USER}" tabindex="{$vt_tab}" class=detailedViewTextBox onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'">
					<input type="hidden" name="{$fldname}" tabindex="{$vt_tab}" value="{$USER_ID}" tabindex="{$vt_tab}" class=detailedViewTextBox onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'">
				</td>
			{elseif $fldname eq 'ticket' && $MODULE eq 'TraitementIncidents' && $OP_MODE eq 'create_view' }
				<td width=30% align=left class="dvtCellInfo">
			...
			{elseif $fldname eq 'ticket' && $MODULE eq 'TraitementIncidents' && $OP_MODE eq 'edit_view' }
				<td width=30% align=left class="dvtCellInfo">
					<input type="text" name="{$fldname}" tabindex="{$vt_tab}" readonly="true" value="{$fldvalue}" tabindex="{$vt_tab}" class=detailedViewTextBox onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'">
				</td>
			{elseif $fldname eq 'datemodification' && $MODULE eq 'TraitementIncidents' }
				<td width=30% align=left class="dvtCellInfo">
		...
		{elseif $fldname neq 'statut' && ( $uitype eq 15 || $uitype eq 16) && $fldname neq 'destination' } <!-- uitype 111 added for noneditable existing picklist values - ahmed -->
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				{elseif $MODULE eq 'TraitementIncidents' && $STATUT eq 'closed' && $fldname eq 'origine' && $ORIGINE neq ''}
			   		<select name="{$fldname}" disabled class="small" >	
				{elseif $MODULE eq 'TraitementIncidents' && $STATUT eq 'traited' && $fldname eq 'origine' && $ORIGINE neq ''}
				...
				{foreach item=arr from=$fldvalue}
					{if ($MODULE eq 'TraitementIncidents' && $STATUT eq 'closed' && $fldname eq 'origine' && $ORIGINE neq '') || ($MODULE eq 'TraitementIncidents' && $STATUT eq 'traited' && $fldname eq 'origine' && $ORIGINE neq '')}
						...
						{if $arr[0] eq $APP.LBL_NOT_ACCESSIBLE}
							...
							{/if}
							<option value="{$arr[0]}" {$IS_SELECTED}>
								{$arr[0]}
							</option>
						{else}
							{if $arr[1] eq $ORIGINE}
							...
							{else}	
								{assign var="IS_SELECTED" value=''}
							{/if}
							<option value="{$arr[1]}" {$IS_SELECTED}>
                            	{$arr[0]}
                         	</option>
                         {/if}	
					{else}
...
*}					
		...
		{elseif $uitype eq 23 || $uitype eq 5 || $uitype eq 6}
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				{if $uitype eq 6  && $CATEGORY eq 'Incidents'}
