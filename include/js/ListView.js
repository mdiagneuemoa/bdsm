...
function getUserChecked() {
	...
	var count = x.length;
	var select_profile_options ="";
	if(count > 1) {
	 	if(confirm(alert_arr.AJOUTER_UTILISATEUR)) {
			for ( var i = 0; i < x.length -1; i++) {
				select_profile_options = select_profile_options + document.getElementById('profil'+x[i]).value +";";
			}
		}
	 	else {
			return false;
	...
	document.getElementById('allProfileSelectedBoxes').value = select_profile_options;
}

function confirmActivation() {
	if(confirm(alert_arr.ACTIVER_UTILISATEUR)) {
	}
 	else {
		return false;
 	}
...
function mass_edit(obj,divid,module) {
	var select_options = document.getElementById('allselectedboxes').value;
	alert(" select_options "+select_options);
	var x = select_options.split(';');
	var count = x.length;
...
function getCreateViewEntriesUsersGID_js(module,url)
{
        var select_options  =  document.getElementsByName('selected_id');
        var x = select_options.length;
        var viewid =getviewId();
	...
	gstart = url;
        new Ajax.Request(
        	'index.php',
                {queue: {position: 'end', scope: 'command'},
                	method: 'post',
                        postBody:"module="+module+"&action="+module+"Ajax&file=CreateUsersGID&ajax=true&allselobjs="+all_selected+"&selobjs="+idstring+"&"+url+urlstring,
			onComplete: function(response) {
                        	$("status").style.display="none";
                                result = response.responseText.split('&#&#&#');
                                $("ListViewContents").innerHTML= result[2];
...
function massValide(module,statut)
{
        var select_options  =  document.getElementById('allselectedboxes').value;
        var x = select_options.split(";");
	var searchurl= document.getElementById('search_url').value;
        ...
        var alert_str = alert_arr.CONFIRMER_MASS_VALIDER_DEMANDE + count +alert_arr.CONFIRMER_MASS_VALIDER2_DEMANDE;
	...
	if(confirm(alert_str))
	{
		...
		new Ajax.Request(
		      	...
		      	{queue: {position: 'end', scope: 'command'},
	                        method: 'post',
               		        postBody: 'module=TraitementDemandes&action=TraitementDemandesAjax&mode=ajax&file=Massvalidate&return_module='+module+'&statut='+statut+'&idlist='+idstring+searchurl,
	                        onComplete: function(response) {
                	                $("status").style.display="none";
