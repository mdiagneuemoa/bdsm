		...
		for ($i=1; $i<=$noofrows; $i++)
		{
				...
				if($is_admin==true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0 || in_array($fieldname,$field) || $fieldname == '')
				{
					...
					else
					{
						...
						elseif(($module == "HReports" || $module == "Documents") && ($fieldname == 'filelocationtype' || $fieldname == 'filename' || $fieldname == 'filesize' || $fieldname == 'filestatus' || $fieldname == 'filetype' || $fieldname == 'docstatus' || $fieldname == 'rapportstatus'))
						{
							...
							// hodar crm pour récupérer le statut du document
			...
			if(!(is_array($selectedfields) && $selectedfields != ''))
			{
				...
				if ($module=='OrdresMission')
				{
					...
					if ($entity_deleted == 1)
						$links_info .=	"&nbsp;<img src='" . vtiger_imageurl('ko.jpg', $theme) . "' title='Mission Annulée'  border=0>";
				...
				if($module == 'ReportingUMV')
				{
					//$links_info = '<a href="index.php?action=ListView&module=OrdresMission&matricule_field='.$entity_id.'&filterMissionAgent=true"><img src=" '. vtiger_imageurl('edit.png', $theme) . '" title="Détail des Missions"  border=0></a>';
					$links_info = '<a href="index.php?action=ListView&module=ReportingRegie&matricule_field='.$entity_id.'&filterMissionAgent=true"><img src=" '. vtiger_imageurl('edit.png', $theme) . '" title="Détail des Missions"  border=0></a>';
				}
				if($module == 'Reunion')
				{
