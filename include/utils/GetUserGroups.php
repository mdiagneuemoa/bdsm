...
class GetUserGroups { 
	...
	function getAllUserGroupsUsersGID($userid)
	{
		global $adb,$log;
		$log->debug("Entering getAllUserGroups(".$userid.") method...");
		//Retreiving from the user2grouptable
		//$query="select * from vtiger_users2group where userid=?";
		$query = "select distinct U.user_id, G.groupid, G.groupname, G.description
					from vtiger_groups G, users U, (select userid, groupid from vtiger_users2group union select supid userid, groupid from siprod_groupsupcoord) UG
