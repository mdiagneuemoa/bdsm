...
function getSearchListHeaderValues($focus, $module,$sort_qry='',$sorder='',$order_by='',$relatedlist='',$oCv='')
{
	...
	foreach($focus->list_fields as $name=>$tableinfo)
	{
		...
		if($fieldname == 'folderid' && $module == 'HReports'){
			$fieldname = 'foldername';
		}
                ...
                if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0 || in_array($fieldname,$field))
		{
			if($fieldname!='parent_id')
			{
				...
				if($name != $app_strings['Close'])
					if ($module != 'UsersGID' || $fld_name != 'statut')
	...
	if($module == 'Demandes' || $module == 'TraitementDemandes' || $module == 'SuiviDemandes' || $module == 'Incidents' || $module == 'TraitementIncidents' || $module == 'SuiviIncidents' || $module == 'Conventions' || $module == 'TraitementConventions' || $module == 'SuiviConventions') {
		$search_header = array();
		$search_header['matricule'] = getTranslatedString('nom');
		$search_header['ticket'] = getTranslatedString('ticket');
		$search_header['lieu'] = getTranslatedString('lieu');

	}
	if($module == 'OrdresMission') {
		$search_header = array();
		$search_header['matricule'] = getTranslatedString('Nom');
		$search_header['numom'] = getTranslatedString('Numom');
		$search_header['lieu'] = getTranslatedString('lieu');
	...
	if($module == 'PHVInfos') {
		$search_header = array();
		$search_header['pays'] = getTranslatedString('pays');
		$search_header['region'] = getTranslatedString('region');

	}
	if($module == 'Tiers') {
		$search_header = array();
		$search_header['nom'] = getTranslatedString('Nom');
		$search_header['initiales'] = getTranslatedString('Initiales');
	...
	if($module == 'Candidats') {
		$search_header = array();
		$search_header['nom'] = getTranslatedString('Nom');
		$search_header['prenom'] = getTranslatedString('Prenom');
		$search_header['nationalite'] = getTranslatedString('Nationalite');
		

	}
	if($module == 'UsersNomade') {
		$search_header = array();
		$search_header['username'] = getTranslatedString('username');
		$search_header['userfirstname'] = getTranslatedString('userfirstname');
...
function BasicSearch($module,$search_field,$search_string){
	...
	elseif($search_field == 'ticket' && $module == 'Demandes'){
		$column_name='ticket';
		$table_name='DEMANDE';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'matricule' && $module == 'Demandes'){
		$column_name='matricule';
		$table_name='DEMANDE';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'lieu' && $module == 'Demandes'){
		$column_name='lieu';
		$table_name='DEMANDE';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'numom' && $module == 'OrdresMission'){
		$column_name='numom';
		$table_name='OM';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'matricule' && $module == 'OrdresMission'){
		$column_name='matricule';
		$table_name='OM';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'lieu' && $module == 'OrdresMission'){
		$column_name='lieu';
		$table_name='OM';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}

	elseif($search_field == 'ticket' && $module == 'Reunion'){
		$column_name='ticket';
		$table_name='REUNION';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'lieu' && $module == 'Reunion'){
		$column_name='lieu';
		$table_name='OM';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'numom' && $module == 'ReportingRegie'){
		$column_name='numom';
		$table_name='DECOMPT';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'matricule' && $module == 'ReportingRegie'){
		$column_name='matricule';
		$table_name='DECOMPT';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'username' && $module == 'UsersNomade'){
		$column_name='user_name';
		$table_name='users';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";	
	}
	elseif($search_field == 'userfirstname' && $module == 'UsersNomade'){
...
function basicFilter($category)
{
	global $adb;
	$where_clauses = array();
	$url_string = "";

	$pays_val = $_REQUEST['pays_field'];
	$activite_val = $_REQUEST['activite_field'];
	...
	$depart_val = $_REQUEST['depart_field'];
	$agent_val = $_REQUEST['agent_field'];			
	
	if ($category == 'Conventions') {
		if(isset($pays_val) && $pays_val != '') {
			$url_string .= "&pays_field=".$pays_val;
			array_push($where_clauses, "(sigc_type_conventions.groupid like concat( ' %',  $pays_val, ' %' ) 
							       or sigc_type_conventions.groupid like concat( '% ', $pays_val, '%' )  
							       or sigc_type_conventions.groupid = $pays_val)");
		}
			
		if(isset($activite_val) && $activite_val != '') {
			$url_string .= "&activite_field=".$activite_val;
			array_push($where_clauses, " sigc_convention.typeconvention = $activite_val ");
		}
		
		if(isset($statut_val) && $statut_val != '') {
			$url_string .= "&statut_field=".$statut_val;
			array_push($where_clauses, "sigc_convention.statut = ".$adb->quote($statut_val));
		}
		
		if(isset($campagne_val) && $campagne_val != '') {
			$url_string .= "&campagne_field=".$campagne_val;
			array_push($where_clauses, "sigc_convention.campagne = $campagne_val ");
		}
		...
		if(isset($date_start_val) && $date_start_val != '') {
			...
			array_push($where_clauses, "vtiger_crmentity.createdtime >= ".$adb->quote($date_start." 00:00:00"));
		}
		...
		if(isset($date_end_val) && $date_end_val != '') {
			...
			array_push($where_clauses, "vtiger_crmentity.createdtime <= ".$adb->quote($date_end." 23:59:59"));
		}		
	}
	elseif ($category == 'Tiers') {
		if(isset($pays_val) && $pays_val != '') {
			$url_string .= "&pays_field=".$pays_val;
			array_push($where_clauses, "tiers_informations.pays like '%".$pays_val."%' ");
		}
			
		if(isset($activite_val) && $activite_val != '') {
			$url_string .= "&activite_field=".$activite_val;
			array_push($where_clauses, " tiers_informations.typeactivite1 like '%".$activite_val."%' ");
		}
		if(isset($statut_val) && $statut_val != '') {
			$url_string .= "&statut_field=".$statut_val;
			array_push($where_clauses, " tiers_informations.active = $statut_val ");
		}
			
	}
	elseif ($category == 'Demandes') {
		/*if(isset($service_val) && $service_val != '') {
			$url_string .= "&service_field=".$pays_val;
			array_push($where_clauses, "siprod_type_demandes.groupid = $service_val");
		}*/
			
		if(isset($statut_val) && $statut_val != '') {
			$url_string .= "&statut_field=".$statut_val;
			array_push($where_clauses, "DEMANDE.statut ='".$statut_val."' ");
		}
		
				
	}
	elseif ($category == 'ReportingRegie') {
		...
		if($date_start_val != '')
		{
			...
			$d = new DateTime($date_start_val);
			$date_start = $d->format("Y-m-d");
			array_push($where_clauses, "DATE_FORMAT(DECOMPT.datedepart,'%Y-%m-%d') >= ".$adb->quote($date_start));
		
		}		
		if($date_end_val != '') 
		{
			$url_string .= "&date_end=".$date_end_val;
			$d2 = new DateTime($date_end_val);
			$date_end = $d2->format("Y-m-d");
			array_push($where_clauses, "DATE_FORMAT(DECOMPT.datedepart,'%Y-%m-%d') <= ".$adb->quote($date_end));
	...
	elseif ($category == 'ReportingUMV') {
		
		if(isset($depart_val) && $depart_val != '') 
		{
			$url_string .= "&depart_field=".$depart_val;
			array_push($where_clauses, " DECOMPT.codedepartement ='".$depart_val."' ");
		}
		
		if(isset($agent_val) && $agent_val != '') 
		{
			$url_string .= "&agent_field=".$agent_val;
			array_push($where_clauses, " DECOMPT.matricule ='".$agent_val."' ");
		}
...
function basicFilterMissionAgent($matricule_val)
{
	global $adb;
	$where_clauses = array();
	$url_string = "";
		...
		if($matricule_val != '')
		{
			$url_string .= "&matricule_field=".$matricule_val;
			array_push($where_clauses, " OM.mat ='".$matricule_val."' ");
...
function basicFilterReportAgent($matricule_val)
{
	global $adb;
	$where_clauses = array();
	$url_string = "";
		...
		{
			$url_string .= "&matricule_field=".$matricule_val;
			array_push($where_clauses, " DECOMPT.mat ='".$matricule_val."' ");
...
/**
 * basicFilterReportingIncident
 */


function basicFilterReportingIncident()
{
	global $adb;
	$where_clauses = array();
	$url_string = "";
	...
	$date_end_val = $_REQUEST['date_end'];
		...
		if(isset($date_start_val) && $date_start_val != '') {
			...
			$d = new DateTime($date_start_val);
			$date_start = $d->format("Y-m-d");
			array_push($where_clauses, " DATE(date_postage) >= ".$adb->quote($date_start) );
		}
		...
		if(isset($date_end_val) && $date_end_val != '') {
			...
			$d = new DateTime($date_end_val);
			$date_end = $d->format("Y-m-d");
			array_push($where_clauses, " DATE(date_postage) <= ".$adb->quote($date_end) );
		}		
...
function basicFilterReportingConvention()
{
	global $adb;
	$where_clauses = array();
	$url_string = "";
