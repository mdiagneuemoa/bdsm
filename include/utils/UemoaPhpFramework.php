...
function getComptesNatureByCodeBudgetPheb($codebudget)
{
		...
		$sql = "SELECT CODEBUDGET,COMPTENAT,LIBCOMPTENAT FROM [dbo].[UEMOA_LIST_BUDGETS_COMPTENATS]
				WHERE CODEBUDGET = '".$codebudget."'";
			
		$result = sqlsrv_query($connect, $sql);
		$comptenatures['']= 'Choisir...';
		while ($comptenat = sqlsrv_fetch_array($result))
		{
			$comptenatures[$comptenat['COMPTENAT']]= utf8_encode($comptenat['COMPTENAT'].' ('.$comptenat['LIBCOMPTENAT'].')');
		}

		return $comptenatures;
		
}
function getAllComptesNatureByDepartPheb($codedepart)
{
		global $log, $singlepane_view,$adb,$DBPHEB;
		global $app_strings, $mod_strings;
		$message ='';
		...
		$sql = "SELECT DISTINCT COMPTENAT,LIBCOMPTENAT FROM [dbo].[UEMOA_LIST_BUDGETS_COMPTENATS]
				WHERE CODEBUDGET LIKE '".$codedepart."%' ORDER BY 1";
...
function getListBudgetsByDepartPheb($codedepart)
{
		global $log, $singlepane_view,$adb,$DBPHEB;
		global $app_strings, $mod_strings;
		$message ='';
		...
		$sql = "SELECT CODEBUDGET,LIBCODEBUDGET,COMPTENAT,LIBCOMPTENAT FROM [dbo].[UEMOA_LIST_BUDGETS_COMPTENATS]
				WHERE CODEBUDGET LIKE '".$codedepart."%' ORDER BY 1";
			
		$result = sqlsrv_query($connect, $sql);
		$codebudgets['']= 'Choisir...';
		while ($codebudget = sqlsrv_fetch_array($result))
		{
			$codebudgets[$codebudget['CODEBUDGET']]= utf8_encode($codebudget['CODEBUDGET'].' ('.$codebudget['LIBCODEBUDGET'].')');
		}

		return $codebudgets;
		
}
function getListBudgetsPheb()
{
		global $log, $singlepane_view,$adb,$DBPHEB;
		global $app_strings, $mod_strings;
		$message ='';
		...
		$sql = "SELECT CODEBUDGET,LIBCODEBUDGET,COMPTENAT,LIBCOMPTENAT FROM [dbo].[UEMOA_LIST_BUDGETS_COMPTENATS] ORDER BY 1";
