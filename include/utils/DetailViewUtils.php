...
function getDetailViewOutputHtml($uitype, $fieldname, $fieldlabel, $col_fields,$generatedtype,$tabid='',$module='')
{
	...
	global $app_strings;
	global $current_user;
	global $theme,$FORMESJURIDIQUES,$NATUREMISSION,$MOYENTRANSPORT,$TYPEBUDGET,$PAYS,$NATIONNALITE,$DEVISES,$SEXE,$CIVILITE,$SITUATIONFAMILIALE,$SCOLOUAPPRENT,$SALOUCOM,$NATUREACTENAIS,$NATUREPIECEIDENT,$TYPECONTRAT,$CATEGORIESALARIE,$STATUTSALARIE,$LANGUEFORMATION,$MODEDEPAIEMENT,$DIPLOME,$NIVEAUSCOLAIRE,$PERMISCONDUIRE,$MOTIFSDEPART;
	...
	//$dossier = getinfosproject($col_fields['projetid']);
	...
	// Siprod change add
	elseif($uitype == 2 && $fieldname=='datecloture' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$datesignature=$col_fields['datesignature'];
			if ($datesignature!='' && $datesignature!='0000-00-00')
			{
				$delai = $col_fields['delai'];
				$date1 = new DateTime($datesignature);
				$date1->add(new DateInterval('P'.$delai.'M'));
				$datecloture = $date1->format('d-m-Y');
				$label_fld[1]= $datecloture;
			}
	}
	elseif($uitype == 2 && $fieldname=='user' && $module=='TraitementDemandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab1=getUserFullName($label_fld[1]);
			$label_fld[1]= $tab1;
	}
	elseif($uitype == 2 && $fieldname=='montant' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=number_format($label_fld[1], 0, ',', ' ');
			$label_fld[1]= $tab3;
		}
	elseif($uitype == 2 && $fieldname=='organe' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$label_fld[1]= $dossier['organe'];
		}
	elseif($uitype == 16) {

		if(   ($fieldname=='groupid' && $module=='TypeDemandes')
			| ( $fieldname == 'destination' && ($module == 'TraitementDemandes' ||$module == 'TraitementIncidents'  || $module == 'TraitementConventions') )
			| ( $fieldname == 'groupid' && $module == 'TypeIncidents' )
			| ( $fieldname == 'groupid' && $module == 'TypeConventions' )			)
		{
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab1=getGroupName($label_fld[1]);
			$label_fld[1]= $tab1[0];
		}
		
		/*elseif($fieldname=='typedemande' && $module=='Demandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab2=getTypeDemandeName($label_fld[1]);
			$label_fld[1]= $tab2;
		}*/
		elseif($fieldname=='typeincident' && $module=='Incidents'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab2=getTypeIncidentName($label_fld[1]);
			$label_fld[1]= $tab2;
		}
		elseif($fieldname=='typeconvention' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab2=getTypeIncidentName($label_fld[1]);
			$label_fld[1]= $tab2;
		}
		elseif($fieldname=='campagne' && ($module=='Demandes' || $module=='Incidents' || $module=='Conventions')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getCampagneName($label_fld[1]);
			$label_fld[1]= $tab3;
		}	
		elseif($fieldname=='domaine' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getDomaineById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='typeactivite' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getTypeActiviteById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='projetid' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getProjectById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='localite' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getLocaliteById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='statut' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getTranslatedString($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='typeactivite1' && $module=='Tiers'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getTypeActiviteTiersById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='affectorgane' || $fieldname=='affectdepartement' ||  $fieldname=='affectdirection' ||  $fieldname=='affectdivision') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getDepartById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='formejuridique' && $module=='Tiers'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=$FORMESJURIDIQUES[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='natureactenaissance') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
				$tab3=$NATUREACTENAIS[$label_fld[1]];
			$label_fld[1]= $tab3;
		}        
		elseif(($fieldname=='nature') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$NATUREPIECEIDENT[$label_fld[1]];
			$label_fld[1]= $tab3;
		}        
		elseif(($fieldname=='conttypecontrat') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$TYPECONTRAT[$label_fld[1]];
			$label_fld[1]= $tab3;
		}        
		elseif(($fieldname=='contcategorie'  ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$CATEGORIESALARIE[$label_fld[1]];
			$label_fld[1]= $tab3;
		}      
		elseif(($fieldname=='contstatut' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$STATUTSALARIE[$label_fld[1]];
			$label_fld[1]= $tab3;
		}    
		elseif(($fieldname=='langue' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$LANGUEFORMATION[$label_fld[1]];
			$label_fld[1]= $tab3;
		}   
			
		elseif(($fieldname=='diplome' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$DIPLOME[$label_fld[1]];
			$label_fld[1]= $tab3;
		}   
		elseif(($fieldname=='permis'  ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$PERMISCONDUIRE[$label_fld[1]];
			$label_fld[1]= $tab3;
		}   
		elseif(($fieldname=='modedepaiement' ||  $fieldname=='modedepaiement2' ||  $fieldname=='modedepaiement3' ||  $fieldname=='modedepaiement4' ||  
			$fieldname=='modedepaiement5' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$MODEDEPAIEMENT[$label_fld[1]];
			$label_fld[1]= $tab3;
		}  
		elseif(($fieldname=='niveauxscolaires') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$NIVEAUSCOLAIRE[$label_fld[1]];
			$label_fld[1]= $tab3;
		} 
		elseif(($fieldname=='contmotifdepart' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$MOTIFSDEPART[$label_fld[1]];
			$label_fld[1]= $tab3;
		} 
		elseif(($fieldname=='nationalite' ||  
			$fieldname=='perenationalite' ||  $fieldname=='merenationalite' ||  $fieldname=='enfant1nationalite' ||  $fieldname=='enfant2nationalite' ||  
			$fieldname=='enfant3nationalite' ||  $fieldname=='enfant4nationalite' ||  $fieldname=='enfant5nationalite' ||  $fieldname=='enfant6nationalite' ||  
			$fieldname=='conjointnationalite' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$NATIONNALITE[$label_fld[1]];
			$label_fld[1]= $tab3;
		} 
		elseif(($fieldname=='sexe' ||  $fieldname=='enfant1sexe' ||  $fieldname=='enfant2sexe' ||  $fieldname=='enfant3sexe' ||  
			$fieldname=='enfant4sexe' ||  $fieldname=='enfant5sexe' ||  $fieldname=='enfant6sexe' ||  $fieldname=='conjointsexe' ||  $fieldname=='peresexe' ||  
			$fieldname=='meresexe' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$SEXE[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='civilite' ||  $fieldname== 'enfant1civilite' ||  $fieldname=='enfant2civilite' ||  $fieldname=='enfant3civilite' ||
			$fieldname=='enfant4civilite' ||  $fieldname=='enfant5civilite' ||  $fieldname=='enfant6civilite' ||  $fieldname=='conjointcivilite' ||  
			$fieldname=='perecivilite' ||  $fieldname=='merecivilite') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$CIVILITE[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='situationfamiliale') && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$SITUATIONFAMILIALE[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='perescolouapprent' ||  
			$fieldname=='merescolouapprent' ||  $fieldname=='enfant1scolouapprent' ||  $fieldname=='enfant2scolouapprent' ||  $fieldname=='enfant3scolouapprent' ||  
			$fieldname=='enfant4scolouapprent' ||  $fieldname=='enfant5scolouapprent' ||  $fieldname=='enfant6scolouapprent' ||  $fieldname=='conjointscolouapprent' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$SCOLOUAPPRENT[$label_fld[1]];
			$label_fld[1]= $tab3;
		} 
		elseif(($fieldname=='peresaloucom' ||  $fieldname=='meresaloucom' ||  $fieldname=='enfant1saloucom' ||  $fieldname=='enfant2saloucom' ||  $fieldname=='enfant3saloucom' ||  
			$fieldname=='enfant4saloucom' ||  $fieldname=='enfant5saloucom' ||  $fieldname=='enfant6saloucom' ||  $fieldname=='conjointsaloucom' ) && $module=='Agentuemoa'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$SALOUCOM[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		
		/*********************************** NOMADE *****************************************/
		elseif(($fieldname=='natmission' ) && $module=='Demandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$NATUREMISSION[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='modetransport' ) && $module=='Demandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$MOYENTRANSPORT[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
			elseif(($fieldname=='budget' || $fieldname=='budget2' ||  $fieldname=='budget3' ||  $fieldname=='budget4' ||  $fieldname=='budget5') && ($module=='Demandes' || $module=='Reunion')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			if ($label_fld[1]!='')
			$tab3=$TYPEBUDGET[$label_fld[1]];
			$label_fld[1]= $tab3;
		}
		/*elseif($fieldname=='lieu' && $module=='Demandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getLieuById($label_fld[1]);
			$label_fld[1]= $tab3;
		}*/
		elseif($fieldname=='matricule' && $module=='Demandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getNomPrenom($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='timbre' && $module=='OrdresMission'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getTimbreById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='trajet1zone' || $fieldname=='trajet2zone' || $fieldname=='trajet3zone' || $fieldname=='trajet4zone' ||$fieldname=='trajet5zone' ||
				$fieldname=='trajet6zone' || $fieldname=='trajet7zone' || $fieldname=='trajet8zone') && $module=='OrdresMission'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getZoneById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif($fieldname=='statut' && $module=='Demandes'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getStatutById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		
		elseif(($fieldname=='sourcefin' || $fieldname=='sourcefin2' || $fieldname=='sourcefin3' || $fieldname=='sourcefin4' || $fieldname=='sourcefin5') && ($module=='Demandes' || $module=='Reunion')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getSourceFinById($label_fld[1]);
			if ($label_fld[1]!='0000-00-01' && $label_fld[1]!='')
				$label_fld[1]= $tab3.'('.$label_fld[1].')';
			else	
				$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='codebudget' || $fieldname=='codebudget2' || $fieldname=='codebudget3' || $fieldname=='codebudget4' || $fieldname=='codebudget5') && ($module=='Demandes')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getActiviteByCodeBudget($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='codebudget' || $fieldname=='codebudget2' || $fieldname=='codebudget3' || $fieldname=='codebudget4' || $fieldname=='codebudget5') && ($module=='Reunion')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getActiviteByCodeBudgetReunion($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='user_direction') && $module=='UsersNomade'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getDirectionById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='user_categmis') && $module=='UsersNomade'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getCategerieById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		/*elseif(($fieldname=='user_profil') && $module=='UsersNomade'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			//print_r($label_fld);
			//echo "profil=",$label_fld[1];
			$tab3=getProfilById($label_fld[1]);
			$label_fld[1]= $tab3;
		}*/
		elseif($fieldname=='departement' && ($module=='Reunion' || $module=='Transfert')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getDepartById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='direction') && $module=='Transfert'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getDirectionById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='responssuivi' || $fieldname=='regisseur') && ( $module=='Reunion')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getNomPrenom($label_fld[1]);
			$label_fld[1]= $tab3;
		}
		elseif(($fieldname=='timbredc' || $fieldname=='timbrecom') && ( $module=='Reunion')){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getTimbreById($label_fld[1]);
			$label_fld[1]= $tab3;
		}		
		elseif($fieldname=='statut' && ($module=='Reunion')){
					$label_fld[] = getTranslatedString($fieldlabel);
					$label_fld[] = $col_fields[$fieldname];
					$tab3=getStatutReunionById($label_fld[1]);
					$label_fld[1]= $tab3;
				}
		elseif($fieldname=='statut' && ($module=='Transfert')){
					$label_fld[] = getTranslatedString($fieldlabel);
					$label_fld[] = $col_fields[$fieldname];
					$tab3=getStatutTransfertById($label_fld[1]);
					$label_fld[1]= $tab3;
				}
	...
	elseif($uitype == 33) //uitype 33 added for multiselector picklist - Jeri
	{
		...
		if($fieldname=='typeactivite1' && $module=='Tiers'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$tab3=getSecteursActivitesById($label_fld[1]);
			$label_fld[1]= $tab3;
		}
...
//		$pick_query="select $fieldname from vtiger_$fieldname order by $fieldname asc";
		...
		for($j = 0; $j < $noofpickrows; $j++)
		{
			...
			if($fieldname == 'groupid' && ( $module == 'TypeIncidents' || $module == 'TypeConventions')) {
				...
				$options[] = array($pickListValue_label,$pickListValue,$chk_val);
			}
			else {
	...
	elseif($uitype == 19)
	{
		if($fieldname == 'notecontent'|| $fieldname == 'description')
			$col_fields[$fieldname]= decode_html($col_fields[$fieldname]);
		else	
			{
	...
	elseif($uitype == 20 || $uitype == 22 || $uitype == 24) // Armando LC<scher 11.08.2005 -> B'descriptionSpan -> Desc: removed $uitype == 19 and made an aditional elseif above
	{
	...
	elseif($uitype == 21)
	{
		if($fieldname=='politique' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$label_fld[1]= $dossier['politique'];
		}
		elseif($fieldname=='programme' && $module=='Conventions'){
			$label_fld[] = getTranslatedString($fieldlabel);
			$label_fld[] = $col_fields[$fieldname];
			$label_fld[1]= $dossier['programme'];
		}
		else
		{
			$label_fld[] = getTranslatedString($fieldlabel);
    ...
    elseif($uitype == 61)
	{
			...
			$label_fld[] =getTranslatedString($fieldlabel);
			$custfldval='';
			$sql = "select * from sedemandefilerel where crmid = ?";
            $result = $adb->pquery($sql, array($col_fields['record_id']));
			while ($row = $adb->fetchByAssoc($result))
			{
				$attachmentid=$row['demandefileid'];
				$attachquery = "select * from attachments where attachmentsid=?";
					$col_fields[$fieldname] = $adb->query_result($adb->pquery($attachquery, array($attachmentid)),0,'name');
					$filename = $adb->query_result($adb->pquery($attachquery, array($attachmentid)),0,'name');
				...
				if($org_filename != '') 
				{
					...
					if($col_fields['filelocationtype'] == 'E' )
					{
						...
						$custfldval = '<a target="_blank" href ='.$col_fields['filename'].' onclick=\'javascript:dldCntIncrease('.$col_fields['record_id'].');\'>'.$filename.'</a><br>';
						}
						else{
					...
					elseif($col_fields['filelocationtype'] == 'I')
					{
						...
						{
							$custfldval .= '<a href = "index.php?module=uploads&action=downloadfile&return_module='.$col_fields['record_module'].'&fileid='.$attachmentid.'&entityid='.$col_fields['record_id'].'" onclick=\'javascript:dldCntIncrease('.$col_fields['record_id'].');\'>'.$filename.'</a><br>';
						}
						else
						{
							$custfldval .= $col_fields[$fieldname];
			...
			//$attachmentid=$adb->query_result($adb->pquery("select * from sedemandefilerel where crmid = ?", array($col_fields['record_id'])),0,'demandefileid');
	...
	elseif($uitype ==300){
		$label_fld[]=getTranslatedString($fieldlabel);
		$label_fld[] =getUserFullNameByUsername($col_fields[$fieldname]);
	}
...
function getRelatedLists($module,$focus)
{
	...
	// hcrm ajouter de la condition "and presence<>1" dans la requete  28/07/09
	...
	$sql1 = "select * from vtiger_relatedlists where tabid=? and presence<>1 and related_tabid not in (SELECT tabid FROM vtiger_tab WHERE presence = 1) order by sequence";
