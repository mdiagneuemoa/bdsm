...
function fetchUserRole($userid)
{
	...
	global $adb;
	$sql = "select roleid from user2role where userid=?";
...
function getTabsPermission($profileid)
{
	...
	global $persistPermArray;
        ...
        {
                $sql = "select * from vtiger_profile2tab where profileid=? and tabid not in(1,16,7,10,13,14,15,18,19,20,21,22,23,24,26,27)";
                $result = $adb->pquery($sql, array($profileid));
                $tab_perr_array = Array();
                $num_rows = $adb->num_rows($result);
                for($i=0; $i<$num_rows; $i++)
...
function getTabsActionPermission($profileid)
{
	...
	$check = Array();
	$temp_tabid = Array();	
	$sql1 = "select * from vtiger_profile2standardpermissions where profileid=? and tabid not in(16,7,10,13,14,15,18,19,20,21,22,23,24,26,27) order by(tabid)";
...
/** Function to get the vtiger_users first_name and last_name  from the vtiger_username 
  * @param $username -- User Name:: Type varchar
  * @returns$firstname." ".$lastname-- First Name and Last Name:: Type varchar
  *
 */

/*
function getUserNomPrenom($username)
{
	global $log;
	$log->debug("Entering getUserNomPrenom(".$username.") method ...");
	global $adb;
	$sql1 = "select first_name,last_name from vtiger_users where user_name=?";
	$result = $adb->pquery($sql1, array($username));
...
*/
/** Function to get the vtiger_users email1 from the vtiger_last_name and vtiger_first_name  
  * @param $last_name,first_name  : Type varchar
  * @returns $email1-- Email1: Type varchar
  *
 */
function getEmail1($userNomPrenom)
{
	global $log;
	$log->debug("Entering getEmail1(".$userNomPrenom.") method ...");
	global $adb;
	$nomPrenom = explode(' ', $userNomPrenom, 2);
	...
	$log->debug("Exiting getEmail1 method ...");
	return $mailuser;	
}
...
{
	global $log;
	$log->debug("Entering getPassword(".$userid.") method ...");
	global $adb;
	
	$res = $adb->pquery("select user_password from vtiger_users where id =?",array($userid));
	$userpw=$adb->query_result($res,0,'user_password');	
	$log->debug("Exiting getPassword method ...");
	return $userpw;	
}
/*
function getUsername($userid)
{
	...
	$username=$adb->query_result($res,0,'user_name');	
...
function isPermitted($module,$actionname,$record_id='')
{
	...
	if($module == 'Demandes' || $module == 'Incidents')
	{
	
		if($current_user->is_admin=="off" || $current_user->profilid!=20 || !isTraiteurIncident($current_user->id,$record_id) || !isPosteurIncident($current_user->id,$record_id) || !isPosteurDemande($current_user->id,$record_id) || !isTraiteurDemande($current_user->id,$record_id))
		{
			$permission = "no";
		}
...
function isPermittedAction($module,$actionname,$record_id='')
{
	global $current_user;
	...
	if($module == 'Demandes')
	{
		if(  $current_user->is_admin=="on"  || $current_user->profilid==20 ){
			...
			elseif(  isTraiteurDemande($current_user->id,$record_id )) {
				$permission = "yes";
			}
			elseif(  isPosteurDemande($current_user->id,$record_id )) {
				$permission = "yes";
			}
			else
			{
	...
	if( $module == 'Incidents')
		{
			...
			elseif(  isTraiteurIncident($current_user->id,$record_id )) {
				$permission = "yes";
			}
			elseif(  isPosteurIncident($current_user->id,$record_id )) {
				$permission = "yes";
			}
			else
			{
	...
	if( $module == 'Conventions')
		{
			...
			elseif(  isTraiteurConvention($current_user->id,$record_id )) {
				$permission = "yes";
			}
			elseif(  isPosteurConvention($current_user->id,$record_id )) {
				$permission = "yes";
			}
			else
			{
...
function isTraiteurPermittedAction($module,$actionname,$record_id='')
{
	global $current_user;
...
function isPosteurPermittedAction($module,$actionname,$record_id='')
{
	global $current_user;
...
function isTraiteurIncident($userid,$id)
	{
		global $adb,$log;
		$log->debug("Entering select isTraiteurDemande($userid) method.");
		
//		$query = "
//			SELECT count(*) as nb_elements 
...
//			INNER JOIN vtiger_groups g ON ( g.groupid like concat( ' %', siprod_type_incidents.groupid, ' %' ) 
//                                    or g.groupid like concat( '% ',siprod_type_incidents.groupid, '%' )  
//                                    or g.groupid = siprod_type_incidents.groupid)
//			INNER JOIN vtiger_users2group gu ON gu.groupid = g.groupid 
//			INNER JOIN siprod_users su ON su.userid = gu.userid 
//			WHERE vtiger_crmentity.deleted =0 
//			And siprod_incident.incidentid=? 
//			And su.userid=?";	
...
//			WHERE vtiger_crmentity.deleted =0 
//			And siprod_incident.incidentid=? 
//			And (g.groupid in (select groupid from vtiger_users2group where userid = ?) 
//				or g.groupid in (select groupid from siprod_groupsupcoord where supid = ?))";	
		
		 $query="SELECT count(*) as nb_elements 
				FROM
				(
				SELECT DISTINCT siprod_incident.ticket, siprod_incident.typeincident, siprod_type_incidents.nom, siprod_incident.statut,
				vtiger_crmentity.createdtime, siprod_incident.campagne, vtiger_crmentity.crmid,siprod_incident.popimpactee, 
				siprod_incident.relanced, siprod_incident.modefonctionnement
				FROM siprod_incident 
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = siprod_incident.incidentid 
				INNER JOIN siprod_type_incidents ON siprod_type_incidents.typeincidentid = siprod_incident.typeincident 
				INNER JOIN vtiger_groups g ON instr(concat(' ', siprod_type_incidents.groupid, ' '), concat(' ', g.groupid, ' ')) > 0
				...
				And (   g.groupid in (select groupid from vtiger_users2group where userid = ?) 
					or 
					g.groupid in (select groupid from siprod_groupsupcoord where supid = ?)
				    )
				...
				INNER JOIN siprod_type_incidents ON siprod_type_incidents.typeincidentid = siprod_incident.typeincident 
				where  vtiger_crmentity.deleted =0
				and (siprod_traitement_incidents.statut = 'transfered'  or siprod_traitement_incidents.statut='pending')
				and (siprod_incident.statut='transfered' or siprod_incident.statut='pending')
				And siprod_incident.incidentid=?
				and  (  siprod_traitement_incidents.destination in (select groupid from vtiger_users2group where userid = ?) 
					or siprod_traitement_incidents.destination in (select groupid from siprod_groupsupcoord where supid = ?))
				) as matable";
		
		$result = $adb->pquery($query, array($id, $userid, $userid,$id, $userid, $userid));
		$noofrows = $adb->query_result($result,0,"nb_elements");
		
		return $noofrows>0?true:false;
	}

	function isPosteurIncident($userid,$demandeid)
	{
		global $adb,$log;
		...
		And  users.user_id=?
		And siprod_incident.incidentid=?";
		$result = $adb->pquery($query, array($userid,$demandeid));
		$noofrows = $adb->query_result($result,0,"nb_elements");
		
		return $noofrows>0?true:false;
	}	
	
	function isPosteurDemande($userid,$id)
	{
		global $adb,$log;
		$log->debug("Entering select isTraiteurDemande($userid) method.");
		...
		$query = "
			...
			INNER JOIN users ON users.user_id = vtiger_crmentity.smcreatorid
			WHERE vtiger_crmentity.deleted =0 
			And siprod_demande.demandeid=?
			And  users.user_id=? ";		
		$result = $adb->pquery($query, array($id, $userid));
		$noofrows = $adb->query_result($result,0,"nb_elements");
		
		return $noofrows>0?true:false;
	}
	function isTraiteurDemande($userid,$id)
	{
		global $adb,$log;
		$log->debug("Entering select isTraiteurDemande($userid) method.");
		...
		/*$query = "
			...
			INNER JOIN vtiger_groups g ON  siprod_type_demandes.groupid=g.groupid 
			INNER JOIN vtiger_users2group gu ON gu.groupid = g.groupid
			INNER JOIN siprod_users su ON su.userid = gu.userid
			WHERE vtiger_crmentity.deleted =0 
			And siprod_demande.demandeid=?
		...
		$query = "
			SELECT count(*) as nb_elements FROM siprod_demande 
			INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = siprod_demande.demandeid 
			INNER JOIN siprod_type_demandes ON siprod_type_demandes.typedemandeid = siprod_demande.typedemande 
			INNER JOIN vtiger_groups g ON siprod_type_demandes.groupid=g.groupid
			WHERE vtiger_crmentity.deleted =0 And siprod_demande.demandeid=?
			And g.groupid in (select groupid from vtiger_users2group where userid = ?) 
			or g.groupid in (select groupid from siprod_groupsupcoord where supid = ?)";	
		$result = $adb->pquery($query, array($id, $userid, $userid));			
			**/
				
		$query = " SELECT count(*) as nb_elements  
					FROM 
					(SELECT DISTINCT siprod_demande.ticket, siprod_demande.typedemande, siprod_type_demandes.nom, siprod_demande.statut,
					vtiger_crmentity.createdtime, siprod_demande.campagne, vtiger_crmentity.crmid, 
					...
					FROM siprod_demande 
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = siprod_demande.demandeid 
					INNER JOIN siprod_traitement_demandes ON siprod_traitement_demandes.ticket = siprod_demande.ticket 
					INNER JOIN siprod_type_demandes ON siprod_type_demandes.typedemandeid = siprod_demande.typedemande 
					where  vtiger_crmentity.deleted =0
					and (siprod_traitement_demandes.statut = 'transfered'  or siprod_traitement_demandes.statut='pending')
					and (siprod_demande.statut='transfered' or siprod_demande.statut='pending')
					And siprod_demande.demandeid=?
					and  (  siprod_traitement_demandes.destination in (select groupid from vtiger_users2group where userid = ?) 
						or siprod_traitement_demandes.destination in (select groupid from siprod_groupsupcoord where supid = ?))
					) as matable ";		
...
function getAllRapportsFolderDetails()
{
global $log;
$log->debug("Entering getAllRapportsFolderDetails() method ...");
	global $adb;
	$folder_det = Array();
	...
	for($i=0; $i<$num_rows;$i++)
	{
		$each_folder_det = Array();
		$folderid=$adb->query_result($result,$i,'folderid');
		$foldername=$adb->query_result($result,$i,'foldername');
		$folderdepth=$adb->query_result($result,$i,'depth');
		$sub_folderdepth=$folderdepth + 1;
		$parentfolder=$adb->query_result($result,$i,'parentfolder');
		$sub_folder='';
		
		//getting the immediate subordinates
		$query1="select * from vtiger_rapportsfolder where parentfolder like ? and depth=?";
		$res1 = $adb->pquery($query1, array($parentfolder."::%", $sub_folderdepth));
		$num_folders = $adb->num_rows($res1);
		if($num_folders > 0)
		{
			for($j=0; $j<$num_folders; $j++)
			{
				if($j == 0)
				{
					$sub_folder .= $adb->query_result($res1,$j,'folderid');
				}
				else
				{
					$sub_folder .= ','.$adb->query_result($res1,$j,'folderid');
		...
		$each_folder_det[]=$foldername;
		$each_folder_det[]=$folderdepth;
		$each_folder_det[]=$sub_folder;
		$folder_det[$folderid]=$each_folder_det;	
		
	}
	$log->debug("Exiting getAllRapportsFolderDetails method ...");
	return $folder_det;
}

function getAllDocumentsFolderDetails()
{
global $log;
$log->debug("Entering getAllDocumentsFolderDetails() method ...");
	global $adb;
	$folder_det = Array();
	$query = "select * from vtiger_attachmentsfolder";
	$result = $adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	for($i=0; $i<$num_rows;$i++)
	{
		...
		//getting the immediate subordinates
		$query1="select * from vtiger_attachmentsfolder where parentfolder like ? and depth=?";
		$res1 = $adb->pquery($query1, array($parentfolder."::%", $sub_folderdepth));
		$num_folders = $adb->num_rows($res1);
		if($num_folders > 0)
		{
	...
	$log->debug("Exiting getAllDocumentsFolderDetails method ...");
	return $folder_det;
}
...
function getRoleUsers($roleId)
{
	...
	global $adb;
	$query = "select user2role.*,vtiger_users.user_name from user2role inner join vtiger_users on vtiger_users.id=user2role.userid where roleid=?";
	$result = $adb->pquery($query, array($roleId));
	$num_rows=$adb->num_rows($result);
...
function getRoleAndSubordinateUsers($roleId)
{
	...
	$query = "select user2role.*,vtiger_users.user_name from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like ?";
...
function deleteRole($roleId,$transferRoleId)
{
	...
	$log->debug("Entering deleteRole(".$roleId.",".$transferRoleId.") method ...");
        ...
        foreach($roleInfo as $roleid=>$roleDetArr)
        {
                ...
                $sql1 = "update user2role set roleid=? where roleid=?";
...
function getAllUserNomPrenom()
{
	global $log;
	$log->debug("Entering getAllUserName() method ...");
	global $adb;
//	$query="select user_id, user_name, user_firstname from users where user_locked=0 and user_statut=1 and user_matricule not like '%E20%' order by user_name";
//	$query="select user_id, user_name, user_firstname from users, siprod_users where siprod_users.userid = users.user_id and user_locked=0 and user_statut=1 and user_matricule not like '%E20%' order by user_name";
	$query="select distinct users.user_id, users.user_name, users.user_firstname
			from users
			right join siprod_users 
			on users.user_id = siprod_users.userid
			where users.user_locked=0 and access_level > 0 
			order by user_name ";
	...
	for($i=0;$i<$num_rows;$i++)
	{
		$userid=$adb->query_result($result,$i,'user_id');
		$username=$adb->query_result($result,$i,'user_name');
		$user_firstname=$adb->query_result($result,$i,'user_firstname');
		$user_details[$userid]=$username." ".$user_firstname;
...
/** Function to insert group to user relation 
  ...
  * @param $userId -- User Id :: Type varchar 
 */
function insertGroupSupCoordRelation($groupId, $supId, $coordId)
{
	global $log;
	$log->debug("Entering insertGroupSupCoordRelation(".$groupId.",".$supId.",".$coordId.") method ...");
	global $adb;
	$query="insert into siprod_groupsupcoord values(?,?,?)";
	$adb->pquery($query, array($groupId, $supId, $coordId));
	$log->debug("Exiting insertGroupSupCoordRelation method ...");
}
...
function getGroupInfo($groupId)
{
	...
	$groupDetailArr[]=getGroupSupCoord($groupId);
...
/** Function to get the group Information of the specified group 
  ...
  *   $groupDetailArray=Array($groupName,$description,$groupMembers);
 */
function getGroupSupCoordInfo($groupId)
{
	global $log;
	$log->debug("Entering getGroupSupCoordInfo(".$groupId.") method ...");
	global $adb;
	$groupDetailArr=Array();
	$groupMemberArr=Array();
	//Retreving the group Info
	...
	//Returning the Group Detail Array
	$log->debug("Exiting getGroupSupCoordInfo method ...");
	return $groupDetailArr;
}
...
function getUserNomPrenom($user_id)
{
	global $log;
	$log->debug("Entering getUserNomPrenom(".$user_id.") method ...");
	$log->info("in getUserNomPrenom ".$user_id);
	global $adb;
	$fullUserName = "";
	if($user_id != '')
	{
        ...
        $log->debug("Exiting getUserNomPrenom method ...");
        return $fullUserName;
}
...
{
	global $log;
	$log->debug("Entering getUserInfo(".$user_id.") method ...");
	$log->info("in getUserInfo ".$user_id);
	global $adb;
	$tabUserInfo=array();
	if($user_id != '')
	{
//		$sql = "select user_matricule, user_firstname, user_name from users where user_id=?";
		$sql = "select distinct users.user_id, users.user_matricule, users.user_name, users.user_firstname, vtiger_profile.profileid, vtiger_profile.profilename
					from vtiger_profile, users, siprod_users
					where siprod_users.profilid = vtiger_profile.profileid
		...
		$profilename = $adb->query_result($result,0,"profilename");
		$tabUserInfo = array("user_matricule" =>$user_matricule,"user_firstname" =>$user_firstname,"user_name" =>$user_name,"profileid" =>$profileid,"profilename" =>$profilename);
	}
        $log->debug("Exiting getUserInfo method ...");
        return $tabUserInfo;
}
...
/** Function to get the group related vtiger_users  
  ...
  *  $groupUsers=Array(userid1,userid2,userid3,.....,useridn);
 */
function getGroupSupCoord($groupId)
{
	global $log;
	$log->debug("Entering getGroupSupCoord(".$groupId.") method ...");
	global $adb;
	$supCoordGroup=Array();
	
	$query="select * from siprod_groupsupcoord where groupid=?";
	$result = $adb->pquery($query, array($groupId));
	
	$supId=$adb->query_result($result,0,'supid');
	$coordId=$adb->query_result($result,0,'coordid');
	
	$supCoordGroup=Array("supId"=>"users::".$supId, "supName"=>getUserNomPrenom($supId),"coordId"=>"users::".$coordId, "coordName"=>getUserNomPrenom($coordId));
	
	$log->debug("Exiting getGroupSupCoord method ...");
	return $supCoordGroup;	
			
}

/**
 * Function to verify if te user is saved in the GID users table
 *
 * @param int $user_id
 * @return the row counts
 */
function isSaved($user_id) {
	global $adb;
	$sql = "select siprod_users.userid from siprod_users where siprod_users.userid =? ";
...
function updateGroupSupCoord($groupId,$supId,$coordId)
{
	global $log;
	$log->debug("Entering updateGroupSupCoord(".$groupId.",".$supId.",".$coordId.") method ...");

	deleteGroupSupCoord($groupId);	

	insertGroupSupCoordRelation($groupId,$supId,$coordId);

	$log->debug("Exiting updateGroupSupCoord method ...");	
...
/** Function to delete group to user relation of the  specified group  
  * @param $groupId -- Group Id :: Type integer 
 */
function deleteGroupSupCoord($groupId)
{
	global $log;
	$log->debug("Entering deleteGroupSupCoord(".$groupId.") method ...");
	global $adb;
	$query="delete from siprod_groupsupcoord where groupid=?";
	$adb->pquery($query, array($groupId));
	$log->debug("Exiting deleteGroupSupCoord method ...");
}
...
function getUserProfileGID($userId)
{
	global $log;
	$log->debug("Entering getUserProfile(".$userId.") method ...");
	global $adb;
	$profArr=Array();	
	$sql1 = "select profilid from siprod_users where userid=?";
    $result1 = $adb->pquery($sql1, array($userId));
	
	$profileid=  $adb->query_result($result1,0,"profilid");
	$profArr[]=$profileid;
...
//	$profArr=getUserProfile($userId);
	$profArr=getUserProfileGID($userId);
...
function getListViewSecurityParameter($module)
{
	...
	if($module == 'Leads')
	{
		$sec_query .= " and (
						...
						or vtiger_crmentity.smownerid in(select user2role.userid from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."::%') 
	...
	{
		$sec_query .= " and (vtiger_crmentity.smownerid in($current_user->id) or vtiger_crmentity.smownerid in(select user2role.userid from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."::%') or vtiger_crmentity.smownerid in(select shareduserid from vtiger_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") ";
	...
	elseif($module == 'Calendar')
	{
		...
		if(isset($shared_ids) && $shared_ids != '')
			...
			$condition = null;
		$sec_query .= " and (vtiger_crmentity.smownerid in($current_user->id) $condition or vtiger_crmentity.smownerid in(select user2role.userid from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."::%')";
...
function getSecListViewSecurityParameter($module)
{
	...
	if($module == 'Leads')
	{
		$sec_query .= " and (vtiger_crmentity$module.smownerid in($current_user->id) or vtiger_crmentity$module.smownerid in(select user2role.userid from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."::%') or vtiger_crmentity$module.smownerid in(select shareduserid from vtiger_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
	...
	elseif($module == 'Accounts')
	{
		$sec_query .= " and (vtiger_crmentity$module.smownerid in($current_user->id) or vtiger_crmentity.smownerid in(select user2role.userid from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."::%') or vtiger_crmentity.smownerid in(select shareduserid from vtiger_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
	...
	elseif($module == 'Calendar')
	{
		...
		if(isset($shared_ids) && $shared_ids != '')
			...
			$condition = null;
		$sec_query .= " and (vtiger_crmentity$module.smownerid in($current_user->id) $condition or vtiger_crmentity$module.smownerid in(select user2role.userid from user2role inner join vtiger_users on vtiger_users.id=user2role.userid inner join vtiger_role on vtiger_role.roleid=user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."::%')";
...
function getFieldVisibilityPermission($fld_module, $userid, $fieldname)
{
	...
	if(($is_admin && $current_user->id==1) || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0)
	{
