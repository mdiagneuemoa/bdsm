...
function getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype,$module_name,$mode='', $typeofdata=null)
{
	...
	if($fieldname=='statut' && $value =='') $value = 'validated';
	$custfld = '';
	$ui_type[]= $uitype;
	...
	{	
		...
		{
			if($fieldname == 'heuredebut_relle' || $fieldname == 'heurefin_relle') {
		...
		if($uitype == 6)
		{
			...
			else
			{
				...
				// préchargement d'une minute en avance par rapport à la date courrante
				//$curr_time = date('H:i',(time() + (1 * 60)));
				// préchargement de la date courrante
	...
	elseif($uitype == 16) {
		...
		// siprod
		if(   ( $fieldname == 'groupid' ||$fieldname == 'destination') 
				&& 
				  	(      $module_name == 'TypeDemandes' 
				  		|| $module_name == 'TypeIncidents'  
				        || $module_name == 'TraitementDemandes' 
				        || $module_name == 'TraitementIncidents'
						|| $module_name == 'TypeConventions'
						|| $module_name == 'TraitementConventions'
					)     
			) 
		
		{
		  $pick_query="select * from vtiger_groups where groupid <> 64";
		} 
		/*elseif( $fieldname == 'typedemande' && $module_name == 'Demandes' ){
		  $pick_query="select siprod_type_demandes.typedemandeid, 
					siprod_type_demandes.nom,siprod_type_demandes.delais,siprod_type_demandes.groupid 
					from siprod_type_demandes
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = siprod_type_demandes.typedemandeid
					WHERE vtiger_crmentity.deleted = 0 
					ORDER BY siprod_type_demandes.nom ASC";
		} */
		elseif( $fieldname == 'typeincident' && $module_name == 'Incidents'  ){
		  $pick_query="
					select siprod_type_incidents.typeincidentid, 
					siprod_type_incidents.nom,siprod_type_incidents.delais,siprod_type_incidents.groupid 
					from siprod_type_incidents
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = siprod_type_incidents.typeincidentid 
					WHERE vtiger_crmentity.deleted = 0 
					ORDER BY siprod_type_incidents.nom ASC";
		} 
		elseif( $fieldname == 'typeconvention' && $module_name == 'Conventions'  ){
		  $pick_query="
					select sigc_type_conventions.typeconventionid, 
					sigc_type_conventions.nom,sigc_type_conventions.delais,sigc_type_conventions.groupid 
					from sigc_type_conventions
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = sigc_type_conventions.typeconventionid 
					WHERE vtiger_crmentity.deleted = 0 
					ORDER BY sigc_type_conventions.nom ASC";
		}
		elseif( $fieldname == 'statut' && ($module_name == 'TraitementDemandes' || $module_name == 'TraitementIncidents') ){
		  $pick_query="select * from siprod_statuts";
		} 
		elseif( $fieldname == 'statut' && $module_name == 'TraitementConventions'){
		  $pick_query="select * from sigc_statuts";
		} 
		elseif( $fieldname == 'origine' && ($module_name == 'TraitementDemandes' || $module_name == 'TraitementIncidents') ){
		  $pick_query="select * from siprod_origines";
		}
		elseif( $fieldname == 'campagne' && ( $module_name == 'Demandes' || $module_name == 'Incidents' ) ){
		  $pick_query="select * from operations where Op_Active = 1 group by Op_Lib";
		} 
		elseif( $fieldname == 'modepaiement' && $module_name == 'ExecutionConventions'){
		  $pick_query="select * from sigc_modepaiement order by 2";
		} 
		elseif( $fieldname == 'affectorgane' && $module_name == 'Agentuemoa'){
		  $pick_query="select * from tiers_departements where depth=1 order by 3";
		} 

		elseif( ($fieldname == 'paysnaissance' || $fieldname == 'paysemission' ||$fieldname == 'adresspays' ||
			$fieldname == 'perepaysnaissance' ||$fieldname == 'merepaysnaissance' ||$fieldname == 'enfant1paysnaissance' ||$fieldname == 'enfant2paysnaissance' ||
			$fieldname == 'enfant3paysnaissance' ||$fieldname == 'enfant4paysnaissance' ||$fieldname == 'enfant5paysnaissance' ||$fieldname == 'enfant6paysnaissance' ||
			$fieldname == 'conjointpaysnaissance') && $module_name == 'Agentuemoa'){
		  $pick_query="select * from tiers_pays order by 3";
		} 
		elseif( $fieldname == 'affectdepartement' && $module_name == 'Agentuemoa'){
		  $pick_query="select * from tiers_departements where depth=2 order by 3";
		}
		elseif( $fieldname == 'affectdirection' && $module_name == 'Agentuemoa'){
		  $pick_query="select * from tiers_departements where depth=3 order by 3";
		}
		elseif( $fieldname == 'affectdivision' && $module_name == 'Agentuemoa'){
		$directionAgent = getListDivision($current_user->user_matricule); // '1-1-5';
		  $pick_query="select * from tiers_departements where depth=4 and parentcode like '%".$directionAgent."%' order by 3";
		}
		elseif( $fieldname == 'affectfonction' && $module_name == 'Agentuemoa'){
		  $pick_query="select * from tiers_fonctions  order by 2";
		}
		/*elseif( $fieldname == 'localite' && $module_name == 'Conventions'){
		  $pick_query="select * from sigc_localites order by 2";
		}
	
		elseif( $fieldname == 'domaine' && $module_name == 'Conventions'){
		  $pick_query="select * from sigc_domaines";
		}
		elseif( $fieldname == 'modepayement' && $module_name == 'ExecutionConventions'){
		  $pick_query="select * from sigc_modepaiement order by 2";
		}*/
		// siprod end
		$params = array();		
		...
		if( $fieldname == 'affectorgane' || $fieldname == 'affectdepartement' || $fieldname == 'affectdirection'  || $fieldname == 'affectdivision'  || $fieldname == 'modepaiement' || $fieldname == 'affectfonction' 
			){
			  $pickListValue=decode_html('');        
			  $pickListLabel=decode_html('Choisir...');
			  $options[] = array(getTranslatedString($pickListLabel),$pickListValue,'');
			  if ($fieldname != 'affectorgane' || $fieldname != 'affectdivision')
			  {
				$options[] = array(getTranslatedString('Aucun(e)...'),'','');
			  }
		}
		if( $fieldname == 'paysnaissance' || $fieldname == 'paysemission' ||$fieldname == 'adresspays' ||
			$fieldname == 'perepaysnaissance' ||$fieldname == 'merepaysnaissance' ||$fieldname == 'enfant1paysnaissance' ||$fieldname == 'enfant2paysnaissance' ||
			$fieldname == 'enfant3paysnaissance' ||$fieldname == 'enfant4paysnaissance' ||$fieldname == 'enfant5paysnaissance' ||$fieldname == 'enfant6paysnaissance' ||
			$fieldname == 'conjointpaysnaissance'){
			  $pickListValue=decode_html('');        
			  $pickListLabel=decode_html('Choisir...');
			  $options[] = array(getTranslatedString($pickListLabel),$pickListValue,'');
		...
		for($j = 0; $j < $noofpickrows; $j++)
		{
			...
			{
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("groupid")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("groupname")));
			}
			/*elseif( $fieldname == 'typedemande' && $module_name == 'Demandes' ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("typedemandeid")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("nom")));
			}*/
			elseif( $fieldname == 'typeincident' && $module_name == 'Incidents' ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("typeincidentid")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("nom")));
			}
			elseif( $fieldname == 'typeconvention' && $module_name == 'Conventions' ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("typeconventionid")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("nom")));
			}
			elseif( $fieldname == 'campagne' && ($module_name == 'Incidents' || $module_name == 'Demandes' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("Op_Id")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("Op_Lib")));
			}
			elseif(    ($module_name == 'TraitementIncidents' | $module_name == 'TraitementDemandes' | $module_name == 'TraitementConventions')
					&& ( $fieldname == 'statut' |  $fieldname == 'origine')  ) 
			{
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("statut")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("libelle")));
			}
			elseif( $fieldname == 'modepaiement' && ($module_name == 'ExecutionConventions' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("modepaiementcode")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("modepaiementlib")));
			}
			elseif( $fieldname == 'affectorgane' && ($module_name == 'Agentuemoa' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("organecode")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("organelibelle")));
			}
			elseif( ($fieldname == 'paysnaissance' || $fieldname == 'paysemission' ||$fieldname == 'adresspays' || 
				$fieldname == 'perepaysnaissance' ||$fieldname == 'merepaysnaissance' ||$fieldname == 'enfant1paysnaissance' ||$fieldname == 'enfant2paysnaissance' ||
				$fieldname == 'enfant3paysnaissance' ||$fieldname == 'enfant4paysnaissance' ||$fieldname == 'enfant5paysnaissance' ||$fieldname == 'enfant6paysnaissance' ||
				$fieldname == 'conjointpaysnaissance') && ($module_name == 'Agentuemoa' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("en")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("fr")));
			}
			elseif( $fieldname == 'affectdepartement' && ($module_name == 'Agentuemoa' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("organecode")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("organelibelle")))."(".decode_html($adb->query_result($pickListResult,$j,strtolower("organesigle"))).")";
			}
			elseif( $fieldname == 'affectdirection' && ($module_name == 'Agentuemoa' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("organecode")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("organelibelle")))."(".decode_html($adb->query_result($pickListResult,$j,strtolower("organesigle"))).")";
			...
			elseif( $fieldname == 'affectfonction' && ($module_name == 'Agentuemoa' ) ){
			  $pickListValue=decode_html($adb->query_result($pickListResult,$j,strtolower("code")));        
			  $pickListLabel=decode_html($adb->query_result($pickListResult,$j,strtolower("intitule")));
			}
			// siprod end 
			...
			// siprod
			if(    ( $fieldname == 'groupid' && ($module_name == 'TypeDemandes' || $module_name == 'TypeIncidents' || $module_name == 'TypeConventions') ) 
			    || ( $fieldname == 'statut' && ($module_name == 'TraitementDemandes' || $module_name == 'TraitementIncidents' || $module_name == 'TraitementConventions') ) 
			  /*  || ( $fieldname == 'typedemande' && $module_name == 'Demandes' )*/
			    || ( $fieldname == 'typeincident' && $module_name == 'Incidents' )
			    || ( $fieldname == 'typeconvention' && $module_name == 'Conventions' )
			    || ( $fieldname == 'campagne' && ($module_name == 'Incidents' || $module_name == 'Demandes' || $module_name == 'Conventions') )
			    || ( $fieldname == 'destination' && ($module_name == 'TraitementDemandes' || $module_name == 'TraitementIncidents' || $module_name == 'TraitementConventions') ) 
				|| ( $fieldname == 'modepaiement' && $module_name == 'ExecutionConventions')
				|| ( $fieldname == 'affectorgane' && $module_name == 'Agentuemoa')
				|| ( $fieldname == 'affectdepartement' && $module_name == 'Agentuemoa')
				|| ( $fieldname == 'affectdirection' && $module_name == 'Agentuemoa')
				|| ( $fieldname == 'affectdivision' && $module_name == 'Agentuemoa')
				|| ( $fieldname == 'affectfonction' && $module_name == 'Agentuemoa')
				|| (($fieldname == 'paysnaissance' || $fieldname == 'paysemission' ||$fieldname == 'adresspays' ||
					$fieldname == 'perepaysnaissance' ||$fieldname == 'merepaysnaissance' ||$fieldname == 'enfant1paysnaissance' ||$fieldname == 'enfant2paysnaissance' ||
					$fieldname == 'enfant3paysnaissance' ||$fieldname == 'enfant4paysnaissance' ||$fieldname == 'enfant5paysnaissance' ||$fieldname == 'enfant6paysnaissance' ||
					$fieldname == 'conjointpaysnaissance' ) && $module_name == 'Agentuemoa')
			  )
			 {
				$pickListLabel = to_html($pickListLabel);
				if(isset($_REQUEST['file']) && $_REQUEST['file'] == 'QuickCreate')
					$options[] = array(htmlentities(getTranslatedString($pickListLabel),ENT_QUOTES,$default_charset),$pickListValue,$chk_val );	
				else
					$options[] = array(getTranslatedString($pickListLabel),$pickListValue,$chk_val );
			 }
			 // siprod end
			else{
		...
		if($fieldname == 'modefonctionnement'){
			  $options[] = array('Choisir','','');
			   if($value == 'Predictive')
			    $options[] = array('Predictive','Predictive','selected');
			   else
			   	$options[] = array('Predictive','Predictive','');
			   	
			   if($value == 'Preview')
			    $options[] = array('Preview','Preview','selected');
			   else
			    $options[] = array('Preview','Preview','');
			   
			  if($value == 'RA')
			    $options[] = array('RA','RA','selected');
		      else
		       $options[] = array('RA','RA','');
		...
		if($fieldname == 'floor'){
			  ...
			  if($value == 'Plateau1')
			    $options[] = array(getTranslatedString("Plateau1"), 'Plateau1', 'selected');
			  else
			   	$options[] = array(getTranslatedString("Plateau1"), 'Plateau1', '');
			   	
			  if($value == 'Plateau2')
			    $options[] = array(getTranslatedString("Plateau2"), 'Plateau2', 'selected');
			  else
			    $options[] = array(getTranslatedString("Plateau2"), 'Plateau2', '');
		}
		/*if($fieldname == 'localite'){
			  $options[] = array('Choisir', '', '');
			  $options[] = getAllLocalitesByCountry();
		}*/

		if($fieldname == 'projetid' || $fieldname == 'localite' ||  $fieldname == 'domaine'  || $fieldname == 'typeactivite' || $fieldname == 'sexe' ||
		   $fieldname == 'nombreenfants' ||  $fieldname == 'natureactenaissance' || $fieldname == 'nature' || $fieldname == 'conttypecontrat' ||
		   $fieldname == 'contcategorie' || $fieldname == 'contstatut' || $fieldname == 'langue' || $fieldname == 'contperiodeessai' || $fieldname == 'diplome' ||
		   $fieldname == 'permis' || $fieldname == 'modedepaiement' || $fieldname == 'modedepaiement2' || $fieldname == 'modedepaiement3' || 
		   $fieldname == 'modedepaiement4' || $fieldname == 'modedepaiement5' || $fieldname == 'niveauxscolaires' || $fieldname == 'contmotifdepart' ||
		   $fieldname == 'nationalite' || $fieldname == 'perenationalite' || $fieldname == 'merenationalite' || 
		   $fieldname == 'enfant1nationalite' || $fieldname == 'enfant2nationalite' || $fieldname == 'enfant3nationalite' || 
		   $fieldname == 'enfant4nationalite' || $fieldname == 'enfant5nationalite' || $fieldname == 'enfant6nationalite' || 
		   $fieldname == 'conjointnationalite' || $fieldname == 'sexe' || $fieldname == 'enfant1sexe' || $fieldname == 'enfant2sexe' || 
		   $fieldname == 'enfant3sexe' || $fieldname == 'enfant4sexe' || $fieldname == 'enfant5sexe' || $fieldname == 'enfant6sexe' || $fieldname == 'conjointsexe' ||
		   $fieldname == 'civilite' ||  $fieldname == 'enfant1civilite' || $fieldname == 'enfant2civilite' || 
		   $fieldname == 'enfant3civilite' || $fieldname == 'enfant4civilite' || $fieldname == 'enfant5civilite' || $fieldname == 'enfant6civilite' || $fieldname == 'conjointcivilite' ||	
		$fieldname == 'situationfamiliale' || $fieldname == 'perescolouapprent' || $fieldname == 'merescolouapprent' || $fieldname == 'enfant1scolouapprent' || $fieldname == 'enfant2scolouapprent' || 
		$fieldname == 'enfant3scolouapprent' || $fieldname == 'enfant4scolouapprent' || $fieldname == 'enfant5scolouapprent' || 
		$fieldname == 'enfant6scolouapprent' || $fieldname == 'conjointscolouapprent' || $fieldname == 'peresaloucom' || $fieldname == 'meresaloucom' || $fieldname == 'enfant1saloucom' || $fieldname == 'enfant2saloucom' || 
		$fieldname == 'enfant3saloucom' || $fieldname == 'enfant4saloucom' || $fieldname == 'enfant5saloucom' || 
		$fieldname == 'enfant6saloucom' || $fieldname == 'conjointsaloucom' 
		|| $fieldname == 'quantite' || $fieldname == 'typedemande' || $fieldname == 'typedemande2' || $fieldname == 'typedemande3'
		|| $fieldname == 'typedemande4' || $fieldname == 'typedemande5' || $fieldname == 'quantite2' || $fieldname == 'quantite3'
		|| $fieldname == 'quantite4' || $fieldname == 'quantite5' || $fieldname == 'anneebudgetaire' || $fieldname == 'anneebudgetaire2' 
		|| $fieldname == 'anneebudgetaire3' || $fieldname == 'anneebudgetaire4' || $fieldname == 'anneebudgetaire5' 
		|| $fieldname == 'dipniveau' || $fieldname == 'etab1pays' || $fieldname == 'etab2pays' || $fieldname == 'etab3pays' 
		|| $fieldname == 'etab1formenligne' || $fieldname == 'etab2formenligne'  || $fieldname == 'etab3formenligne'  
		|| $fieldname == 'etab1formpreinscription' || $fieldname == 'etab2formpreinscription'  || $fieldname == 'etab3formpreinscription'
		
				|| $fieldname == 'modetransport' || $fieldname == 'natmission'  || $fieldname == 'matricule'
		|| $fieldname == 'budget' || $fieldname == 'sourcefin'  || $fieldname == 'codebudget' || $fieldname == 'comptenat'
		|| $fieldname == 'budget2' || $fieldname == 'sourcefin2'  || $fieldname == 'codebudget2' || $fieldname == 'comptenat2'
		|| $fieldname == 'budget3' || $fieldname == 'sourcefin3'  || $fieldname == 'codebudget3' || $fieldname == 'comptenat3'
		|| $fieldname == 'budget4' || $fieldname == 'sourcefin4'  || $fieldname == 'codebudget4' || $fieldname == 'comptenat4'
		|| $fieldname == 'budget5' || $fieldname == 'sourcefin5'  || $fieldname == 'codebudget5' || $fieldname == 'comptenat5'
		|| $fieldname == 'matdirecteur' || $fieldname == 'matinterimaire'  
		|| $fieldname == 'timbre' || $fieldname == 'signataire'  
		|| $fieldname == 'trajet1zone' || $fieldname == 'trajet2zone'  || $fieldname == 'trajet3zone' || $fieldname == 'trajet4zone' 
		|| $fieldname == 'trajet5zone' || $fieldname == 'trajet6zone'  || $fieldname == 'trajet7zone' || $fieldname == 'trajet8zone' 
		|| $fieldname == 'user_direction' || $fieldname == 'user_categmis'  || $fieldname == 'user_profil' || $fieldname == 'user_sexe' 
		|| $fieldname == 'responssuivi' || $fieldname == 'departement'  || $fieldname == 'regisseur' || $fieldname == 'lieu' || $fieldname == 'typereamenagement'
		   ){
			 $fieldvalue [] = $value;
		}
		else
	...
	elseif($uitype == 33)
	{
		...
		if($fieldname == 'groupid'  && ( $module_name == 'TypeIncidents' || $module_name == 'TypeConventions')) 
		{
		  $mulsel="select * from vtiger_groups where groupid <> 64";
		} 
		else {
			$mulsel="select $fieldname from vtiger_$fieldname order by $fieldname asc";
		}
//		$mulsel="select $fieldname from vtiger_$fieldname order by $fieldname asc";
		...
		for($j = 0; $j < $noofoptions; $j++)
		{
			...
			if($fieldname == 'groupid' && ( $module_name == 'TypeIncidents' || $module_name == 'TypeConventions')) {
				$multiselect_combo_label = $adb->query_result($multiselect_result,$j,strtolower("groupname"));
				$options[] = array($multiselect_combo_label,$multiselect_combo,$chk_val );
			}
			else {
	...
	elseif($uitype == 61)
	{
		...
		else
		{
			...
			{
				$sql = "select * from nomade_sedemandefilerel where crmid = ?";
				$result = $adb->pquery($sql, array($col_fields['record_id']));
				while ($row = $adb->fetchByAssoc($result))
				{	
					$attachmentid=$row['demandefileid'];	
					$attachquery = "select * from nomade_attachments where attachmentsid=?";
					$value = $value.' <br> '.$adb->query_result($adb->pquery($attachquery, array($attachmentid)),0,'name');
