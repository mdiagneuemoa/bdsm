...
function getListViewEntries($focus, $module,$list_result,$navigation_array,$relatedlist='',$returnset='',$edit_action='EditView',$del_action='Delete',$oCv='',$page='',$selectedfields='',$contRelatedfields='',$skipActions=false)
	...
	if($navigation_array['start'] !=0)
		for ($i=1; $i<=$noofrows; $i++)
		{
			...
			{
				$entity_id  = $adb->query_result($list_result,$i-1,"semaine_postage");
			...
			foreach($focus->list_fields as $name=>$tableinfo)
			{
				...
				if($is_admin==true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0 || in_array($fieldname,$field) || $fieldname == '')
				{
					...
					else
					{
						...
						elseif($fieldname == 'semaine_postage' && $module == 'ReportingIncidents')
						{
							 ...
							 $y= substr($val,0,4);
							 $w= substr($val,4,2);
							 $tmp = get_lundi_dimanche_from_week($w,$y);
							 $value = ' '.$tmp[0].' - '.$tmp[1].' ';
						}
						else
						{
			...
			if(!(is_array($selectedfields) && $selectedfields != ''))
			{
				...
				if($_SESSION['DemandesAtraiter'] == 'true' || $_SESSION['DemandesASuivre'] == 'true' || $_SESSION['IncidentsAtraiter'] == 'true' || $_SESSION['IncidentsASuivre'] == 'true' || $module=='Demandes' || $module=='Incidents') {
					$links_info = "";
				}
...
function getValue($field_result, $list_result,$fieldname,$focus,$module,$entity_id,$list_result_count,$mode,$popuptype,$returnset='',$viewid='')
	...
	elseif($uitype == 33)
	{
		...
		if($fieldname == 'groupid' && $module == 'TypeIncidents') {
	...
	else
	{
		if($fieldname == $focus->list_link_field)
		{
			...
			else
			{
				...
				elseif($module == 'Demandes' || $module == 'TraitementDemandes' || $module == 'Incidents' || $module == 'TraitementIncidents')
				{
