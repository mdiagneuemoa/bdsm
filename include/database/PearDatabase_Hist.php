...
// See function convertPS2Sql in PearDatabase_Hist below
class PreparedQMark2SqlValue_Hist {
	// Constructor
	function PreparedQMark2SqlValue_Hist($vals){
        $this->ctr = 0;
        $this->vals = $vals;
    }
...
class PearDatabase_Hist{
	...
	{
	    $log1->info("PearDatabase_Hist ->".$msg);
	}
	return $msg;
    }
 ...
 */
	...
	function convert2Sql($ps, $vals) {
		...
		$sql = preg_replace_callback("/('[^']*')|(\"[^\"]*\")|([?])/", array(new PreparedQMark2SqlValue_Hist($vals),"call"), $ps); 
		return $sql;
	}
    ...
    ## adds new functions to the PearDatabase_Hist class to come around the whole
 ...
 */
    ...
    function PearDatabase_Hist($dbtype='',$host='',$dbname='',$username='',$passwd='')
    {
	//$this->println("PearDatabase_Hist");
	global $currentModule;
	$this->log =& LoggerManager::getLogger('PearDatabase_Hist_'. $currentModule);
	...
	if($host == '')
	{
	    ...
	    $this->setDatabaseHost( $dbconfig['db_hostname_hist']);
	    $this->setDatabaseName($dbconfig['db_name_hist']);
