...
$tabUserInfo = getUserInfo($record);

$matricule = $tabUserInfo["user_matricule"];
$nom = $tabUserInfo["user_name"];
$prenom = $tabUserInfo["user_firstname"];
$idProfile = $tabUserInfo["profileid"];
$nomProfile = $tabUserInfo["profilename"];

$profilList = getAllProfileGID();

$profileInfo = '<select id="profil'.$record.'" name="profil'.$record.'" class="small">';
$option = '<option value=""></option>';
$profileInfo .= $option;	

foreach($profilList as $profileid=>$profilename) {
	if($idProfile == $profileid) {
			$option = '<option selected value="'.$profileid.'">'.$profilename.'</option>';
	}
	else {
...
$smarty->assign('PROFILESLIST', $profileInfo);
$smarty->assign('IDPROFILE', $idProfile);
$smarty->assign('PROFILENAME', $nomProfile);
...
$smarty->display('EditViewUsersGID.tpl');
