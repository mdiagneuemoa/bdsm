...
function load_webmail(mid) {
	...
	*/
	...
	if($("preview1").style.visibility === "hidden" || $("preview2").style.visibility === "hidden") {
        ...
        $("delete_button").appendChild(Builder.node('input',{type: 'button', name: 'Button', value: 'Supprimer', className: 'buttonok', onclick: 'runEmailCommand(\'delete_msg\','+mid+')'}));

        $("reply_button_all").removeChild($("reply_button_all").firstChild);
        $("reply_button_all").appendChild(Builder.node('input',{type: 'button', name: 'reply', value: ' Repondre a Tous', className: 'buttonok', onclick: 'OpenComposer('+mid+',\'replyall\')'}));

        $("reply_button").removeChild($("reply_button").firstChild);
        $("reply_button").appendChild(Builder.node('input',{type: 'button', name: 'reply', value: ' Repondre ', className: 'buttonok', onclick: 'OpenComposer('+mid+',\'reply\')'}));

        $("forward_button").removeChild($("forward_button").firstChild);
        $("forward_button").appendChild(Builder.node('input',{type: 'button', name: 'forward', value: ' Transferer ', className: 'buttonok', onclick: 'OpenComposer('+mid+',\'forward\')'}));

        $("qualify_button").removeChild($("qualify_button").firstChild);
	if(showQualify == 'yes')
        	$("qualify_button").appendChild(Builder.node('input',{type: 'button', name: 'Qualify2', value: ' Qualifier ', className: 'buttonok', onclick: 'showRelationships('+mid+')'}));
	else
        ...
        $("download_attach_button").appendChild(Builder.node('input',{type: 'button', name: 'download', value: ' Telecharger Fichier Joint ', className: 'buttonok', onclick: 'displayAttachments('+mid+')'}));
		
		$("print_email_button").removeChild($("print_email_button").firstChild);
		$("print_email_button").appendChild(Builder.node('input',{type: 'button', name: 'print', value: ' Imprimer ',className: 'buttonok', onclick: 'OpenComposer('+mid+',\'print\')'}));
