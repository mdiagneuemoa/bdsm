...
if($_REQUEST['filter'] == 'true') {
	list($where, $ustring) = split('#@@#', basicFilter('ReportingRegie'));
	$url_string .= "&filter=true$ustring";
	$smarty->assign('SEARCH_URL', $url_string);
}

if($_REQUEST['filterMissionAgent'] == 'true') {
	list($where, $ustring) = split('#@@#', basicFilterReportAgent($_REQUEST['matricule_field']));
	$url_string .= "&filter=true$ustring";
	$smarty->assign('SEARCH_URL', $url_string);
}
// Custom View
...
if ($curentuserprofil==20 ||  $curentuserprofil==30 ||  $curentuserprofil==29 || ($curentuserprofil==21 && $currentuserdirection=='01-02-124'))
{
$list_query = "SELECT DECOMPT.omid,DECOMPT.numom,DECOMPT.matricule,DECOMPT.lieu,DECOMPT.objet,DECOMPT.datedepart,DECOMPT.datearrivee,DECOMPT.duree,DECOMPT.mat,
				DECOMPT.codebudget,SUM(DECOMPT.mnttotal) AS totaldecompte
				FROM
				(SELECT nomade_ordremission.omid,nomade_ordremission.numom,CONCAT(users.user_name,'  ',users.user_firstname) AS matricule,users.user_matricule  AS mat,
				nomade_demande.lieu,nomade_demande.objet,nomade_ordremission.datedepart,nomade_ordremission.datearrivee,
...
/* WHERE nomade_decompte.codebudget!='' */	
if($where != '') {
	$list_query = "$list_query WHERE $where";
}
	$list_query .=" GROUP BY DECOMPT.codebudget,DECOMPT.numom ORDER BY DECOMPT.numom DESC ";
