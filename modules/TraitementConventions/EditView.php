...
$smarty->assign("RESERVER", $_REQUEST['reserver']);
...
if(getFieldVisibilityPermission('TraitementIncidents',$current_user->id,'description') != '0')
        $FCKEDITOR_DISPLAY = false;
$FCKEDITOR_DISPLAY = true;
$smarty->assign("FCKEDITOR_DISPLAY",$FCKEDITOR_DISPLAY);

$idd = $_REQUEST['record'];
$libconv = $_REQUEST['libconv'];
$statut = $_REQUEST['statut'];
$smarty->assign('TICKET', (isset ($_REQUEST['ticket']) && $ticket != '') ? $ticket : getTicketConvention($idd));
$smarty->assign('LIBELLECONV', (isset ($_REQUEST['libconv']) && $libconv != '') ? $libconv : getLibelleConvention($idd));
$smarty->assign('STATUT', $statut);

if ($focus->mode == 'edit') {
	$smarty->display('salesEditView.tpl');
} else {
	$smarty->assign("ORIGINE", getOrigineIncient($ticket));
	//$smarty->display('CreateViewTraitementConventions.tpl');
