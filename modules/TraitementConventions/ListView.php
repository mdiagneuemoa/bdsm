...
if( $currentModule == 'TraitementConventions' )
	$currentModule = 'Conventions';
elseif( $currentModule == 'Conventions' )
	$currentModule = 'TraitementConventions';
...
require_once('modules/TraitementConventions/TraitementConventions.php');
...
$list_query .=" AND sigc_convention.statut NOT IN('validated','completed','running') AND deleted =0 ";
...
$list_query="select sigc_convention.ticket,sigc_convention.libellecourt,sigc_convention.statut,sigc_convention.montant,sigc_convention.datedemarrage,
			  sigc_convention.delai,sigc_convention.agenceexecution ,vtiger_crmentity.crmid 
			  FROM sigc_convention 
			  INNER JOIN vtiger_crmentity ON crmid = sigc_convention.conventionid ";
$list_query .=	$join_clause;
$list_query .="	WHERE deleted =0 ";
$list_query .=" AND sigc_convention.statut NOT IN('validate','running','complete','running') ";
	*/		 
if($where != '') {
	$list_query = "$list_query AND $where";
}

$record1 = $_REQUEST['record'];
if($record1 != '' && $currentModule=='TraitementConventions') {
	$ticket= getTicketConvention($record1);
	$list_query .=" AND sigc_traitement_conventions.ticket=" .$ticket ;
}
...
echo $start, $recordCount, $list_max_entries_per_page;
