...
function getIncidentsATraites($maxval,$calCnt)
{
	...
	$where = " AND (sigc_convention.statut='pending' or sigc_convention.statut='open' or sigc_convention.statut='transfered' or sigc_convention.statut='reopen') ";
	...
	if($current_user->isTraiteurIncident($current_user->id) > 0 ||  $current_user->is_admin == 'on' || $current_user->profilid == 20 ) {
		...
		$where = " AND (sigc_convention.statut='pending' or sigc_convention.statut='reopen' or sigc_convention.statut='open' or sigc_convention.statut='transfered') ";
		
		$list_query = "	SELECT distinct vtiger_crmentity.crmid, vtiger_crmentity.createdtime,sigc_convention.ticket,sigc_convention.libelle,sigc_convention.statut,sigc_convention.montant,
	...
	elseif($current_user->isPosteurIncident($current_user->id) > 0 ||  $current_user->is_admin == 'on' || $current_user->profilid == 20 ) {
		...
		$list_query = "	SELECT distinct vtiger_crmentity.crmid, vtiger_crmentity.createdtime,
				sigc_convention.ticket,sigc_convention.libelle,sigc_convention.statut,sigc_convention.montant,
				sigc_convention.datedemarrage,sigc_convention.organe,sigc_convention.bailleurs,
				sigc_convention.maitriseouvrage,sigc_convention.agenceexecution,sigc_convention.beneficiaire,
			sigc_conventioncf.*
			FROM sigc_convention
	...
	if (isset($incidents_a_traiter_view_all_check) && ($incidents_a_traiter_view_all_check == 'Incidents_a_traiter_view_all_check' || $incidents_a_traiter_view_all_check == 'on')) {
		...
		$where = " AND (sigc_convention.statut='pending' or sigc_convention.statut='open') ";
...
//				sigc_type_conventions.nom as lbtypeincident,sigc_convention.popimpactee,
//				sigc_conventioncf.* 
//				
//				FROM sigc_convention 
//				
//				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = sigc_convention.conventionid 
//				INNER JOIN sigc_type_conventions ON sigc_type_conventions.typeconventionid = sigc_convention.typeincident 
