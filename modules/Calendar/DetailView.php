 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /cvs/repository/siprodPCCI/modules/Calendar/DetailView.php,v 1.1 2010/01/15 18:43:49 isene Exp $
...
$usr_res = $adb->pquery("select count(*) as count from vtiger_users where user_name=?",array($finaldata['assigned_user_id']));
if($adb->query_result($usr_res, 0, 'count')>0) {
	$assignType = "U";
	$data['assigned_user_id'] = getUserFullNameByUsername($finaldata['assigned_user_id']);
}
else {
	$assignType = "T";
...
$data['chefequipe'] = getUserFullNameByUsername($finaldata['chefequipe']);
$data['coordonnateur'] = getUserFullNameByUsername($finaldata['coordonnateur']);
$data['consultant1'] = getUserFullNameByUsername($finaldata['consultant1']);
$data['consultant2'] = getUserFullNameByUsername($finaldata['consultant2']);
$data['assistant1'] = getUserFullNameByUsername($finaldata['assistant1']);
$data['assistant2'] = getUserFullNameByUsername($finaldata['assistant2']);
...
elseif($activity_mode == 'Events')
{
	...
	$sql = 'select vtiger_users.user_name,vtiger_users.first_name,vtiger_users.last_name,vtiger_invitees.* from vtiger_invitees left join vtiger_users on vtiger_invitees.inviteeid=vtiger_users.id where activityid=?';
	...
	for($i=0;$i<$num_rows;$i++)
	{
		...
		$invited_users[$userid]=$firstname." ".$lastname;
	}
...
// hodar crm cette partie permet de ne pas autoriser la modif ou sup d'une tache qui nous est assigné par un supérieur
$can_edit_delete = can_Update_Delete_Activities($focus->id,$finaldata['assigned_user_id'],$current_user->id);
//get Description Information
if(isPermitted("Calendar","EditView",$_REQUEST['record']) == 'yes' && $can_edit_delete==true)
	$smarty->assign("EDIT_DUPLICATE","permitted");

if(isPermitted("Calendar","Delete",$_REQUEST['record']) == 'yes' && $can_edit_delete==true)
