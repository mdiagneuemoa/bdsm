							...
							<td>
								...
								<table class="small" border="0" cellpadding="3" cellspacing="0" width="100%">
									<tr>
										...
										<td class="$todotab_class" style="width:100px;" id="pi" align="center" nowrap="nowrap">$todo_anchor</td>
										<td class="dvtTabCache" style="width: 10px;" nowrap="nowrap">&nbsp;</td>
										<td class="$eventtab_class" id="mi" align="center" nowrap="nowrap" width="75">$event_anchor</td>
...
function get_cal_header_data(& $cal_arr,$viewBox,$subtab)
{
	...
	$headerdata .="
			...
			$headerdata .="<tr>
				...
				<table>
					<tr>
					<td class='calAddButton' style='border: 1px solid #666666;cursor:pointer;height:40px' align='center' width='120'>
					<a href='javascript:void(0);' onClick='gshow(\"addEvent\",\"Call\",\"".$temp_date."\",\"".$endtemp_date."\",\"".$time_arr['starthour']."\",\"".$time_arr['startmin']."\",\"".$time_arr['startfmt']."\",\"".$time_arr['endhour']."\",\"".$time_arr['endmin']."\",\"".$time_arr['endfmt']."\",\"listview\",\"event\");'>"
					.$mod_strings['LBL_ADD']." ".$app_strings['Meeting']."</a></td>
					
					
				
				</tr></table> </td>";
				/*
					<td class='calAddButton' style='cursor:pointer;height:30px' align='center' width='15%' onMouseOver='fnAddEvent(this,\"addEventDropDown\",\"".$temp_date."\",\"".$temp_date."\",\"".$time_arr['starthour']."\",\"".$time_arr['startmin']."\",\"".$time_arr['startfmt']."\",\"".$time_arr['endhour']."\",\"".$time_arr['endmin']."\",\"".$time_arr['endfmt']."\",\"".$viewBox."\",\"".$subtab."\",\"".$eventlist."\");'>
					".$mod_strings['LBL_ADD']."
	...
	$headerdata .="<td  ><span id='total_activities'>";//USER SELECT CUSTOMIZATION
	$headerdata .= getEventInfo($cal_arr,'listcnt'); 
	$headerdata .= "</span></td>";
	//$headerdata .= "<td align='center' width='50%'><table border=0 cellspacing=0 cellpadding=2><tr><td class=small><b>".$mod_strings['LBL_VIEW']." : </b></td><td>";//USER SELECT CUSTOMIZATION
	...
	//$headerdata .=$view_options."</td></tr></table></td>";
	$headerdata .="</tr></table></form>";
	echo $headerdata;	
...
function getEventList(& $calendar,$start_date,$end_date,$info='')
{
	...
	for($i=0;$i<$rows;$i++)
	{
		...
		if(!empty($assignedto))
			$element['assignedto'] = getUserNomPrenom($assignedto);
		else
   ...
   $query = "SELECT vtiger_groups.groupname, vtiger_users.user_name, vtiger_crmentity.crmid, vtiger_cntactivityrel.contactid, 
	...
	// END
        ...
        if($info != '')
		{
			...
			$pending_query = $query." AND (vtiger_activity.status != 'clôturé')".$com_q;
			$total_q =  $query."".$com_q;
			array_push($info_params, $current_user->id);
			...
			{	
				...
				if($key!='visibility'){
					if(($key=='eventdetail'|| $key=='action') && ($current_user->column_fields['is_admin']!='on' && $adb->num_rows($role_list)==0 && (($adb->num_rows($is_shared)==0 && ($entry_list[$i]['visibility']=='Public' || $entry_list[$i]['visibility']=='Private')) || $entry_list[$i]['visibility']=='Private')) && $current_user->column_fields['user_name']!=$entry_list[$i]['assignedto']){
						if($key=='eventdetail')
							$list_view .="<td nowrap='nowrap'><font color='red'><b>".getUserNomPrenom($entry_list[$i]['assignedto'])." - ".$mod_strings['LBL_BUSY']."</b></font></td>";
						else
...
function constructTodoListView($todo_list,$cal,$subtab,$navigation_array='')
{
	...
	$list_view .="<table align='center' border='0' cellpadding='5' cellspacing='0' width='98%'>
			...
			*/
            ...
            $list_view .="<tr>
				<td class='calAddButton' style='border: 1px solid #666666;cursor:pointer;height:30px' align='center' width='10%'>
					<a href='javascript:void(0);' onClick='gshow(\"createTodo\",\"todo\",\"".$temp_date."\",\"".$temp_date."\",\"".$time_arr['starthour']."\",\"".$time_arr['startmin']."\",\"".$time_arr['startfmt']."\",\"".$time_arr['endhour']."\",\"".$time_arr['endmin']."\",\"".$time_arr['endfmt']."\",\"listview\",\"todo\");'>"
					.$mod_strings['LBL_ADD']." ".$app_strings['Todo']."</a></td>";
