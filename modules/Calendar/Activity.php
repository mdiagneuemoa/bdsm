 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /cvs/repository/siprodPCCI/modules/Calendar/Activity.php,v 1.1 2010/01/15 18:43:49 isene Exp $
	...
	/** Function to insert values in vtiger_salesmanactivityrel table for the specified module
 	 ...
 	 */
  	...
  	function insertIntoSmActivityRel($module)
  	{
		...
		// hodar CRM gestion des assignations multiples	
	/*	$listAssignedUsers = $this->column_fields['assigned_user_id'];
		foreach($listAssignedUsers as $user_id)
		{
			$user_sql = $adb->pquery("select count(*) as count from vtiger_users where id=?", array($user_id));
	    	if($adb->query_result($user_sql, 0, 'count') != 0) 
			{
				$sql_qry = "insert into vtiger_salesmanactivityrel (smid,activityid) values(?,?)";
	    		$adb->pquery($sql_qry, array($user_id, $this->id));
