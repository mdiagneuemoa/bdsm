...
$sfiche = $_REQUEST['sfiche'];
if ($sfiche == '') $sfiche = 'SFPR23-01';
$infosfiches = $focus->getInfosFiche($sfiche);
//print_r($infosfiche);
$tree = $infosfiches['categorie'].' > '.$infosfiches['libfiche'];
$sfiches = $infosfiches['sfiches'];
$smarty->assign('MENUTREE', $tree);
$smarty->assign('SFICHES', $sfiches);
$smarty->assign('CODESFICHE', $sfiche);
$smarty->assign('DESCSFICHE', $sfiches[$sfiche]['designation']);
...
//$datasInd = $focus->getDonneesIndicateursMensuels($sfiche,'SN',1,12,2008,2009);
$datasInd = $focus->getDonneesIndicateursAnnuels($sfiche,'SN',2016,2018);
//$datasInd = $focus->getDonneesIndicateursTrimestriels($sfiche,'SN',2008,2009);
$smarty->assign("INDICATEURSDATAS", $datasInd);
$smarty->assign("SFICHE", $focus->getLibelleSfiche($sfiche));
$smarty->assign("SFICHEID", $sfiche);
$smarty->assign("SFICHEINFOS", $focus->getInfosSfiche($sfiche));
...
$smarty->assign("ANNEESTAT", $ANNEESTAT);
$smarty->assign("MOISSTAT", $MOISSTAT);
$smarty->assign("MOISSTATS", $MOISSTATS);
$smarty->assign("TRIMESTRESTATS", $TRIMESTRESTATS);
...
if(isset($_REQUEST['ajax']) && $_REQUEST['ajax'] != '')
	$smarty->display("StatisticsSMListViewEntries.tpl");
else 
	$smarty->display('StatisticsSMListView.tpl');
