...
$smarty->assign("TRIMESTRESTATS", $TRIMESTRESTATS);	  
  ...
  $smarty->assign("CFICHES", $focus->getAllCategoriesFiches());	  
  $smarty->assign("FICHES", $focus->getAllFiches());	  
   $smarty->assign("SFICHES", $focus->getAllSousFiches());	  
//$smarty->assign("SFICHEINFOS", $focus->getInfosSfiche($sfiche));

if($focus->mode == 'edit') {
	$smarty->display('StatisticsSMEditView.tpl');
} else {
		$smarty->display('StatisticsSMCreateView.tpl');
}
