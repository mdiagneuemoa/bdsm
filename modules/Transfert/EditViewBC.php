...
if($record) {
	...
	$ticketreunion = $focus->getTicketReunion($record);
	$smarty->assign("TICKET",$ticketreunion);
	$nextTicketReunionBC=$focus->getNextTicketReunionBC($ticketreunion);
	$smarty->assign('NEXTREUNIONBC', $nextTicketReunionBC );
	//$smarty->assign('NEXTNUMEROCONVENTION', $matriculeconv );
	//echo $ticketreunion,' - ',$nextTicketReunionBC;
	$smarty->assign('OP_MODE','openbc');
...
if($disp_view == 'edit_view'  || $focus->mode == 'openbc') 
...
$smarty->assign("FCKEDITOR_DISPLAY",$FCKEDITOR_DISPLAY);
 ...
 $smarty->assign("DISPLAY_JUSTIFS2",$display_justifs2);	 
  ...
  $smarty->assign("TYPESBUDGET",$focus->getTypeBudget());	
  $smarty->assign("SOURCESFINACEMENT",$focus->getSourcesFinancement());	
  //$smarty->assign("CODESBUDGETAIRES",getCodesBudgetaires());	
  $smarty->assign("CODESBUDGETAIRES",getBudgetsReunionByAgentDepart($current_user->user_matricule));	
  ...
  $smarty->assign("VILLESUEMOA", getAllVillesPays());	  
  $smarty->assign("LISTREGISSEURS", $focus->getAllRegisseurs());	  
//  $ticket = $focus->column_fields['ticket'];
  
	$smarty->display('ReunionEditViewBC.tpl');
