...
//$list_query="select user_id, user_matricule, user_numerologin, user_name, user_firstname from users where user_locked=0 and user_statut=1 and user_matricule not like '%E20%' and user_id not in (select userid from  siprod_users )";
$list_query="select user_id, user_matricule, user_numerologin, user_name, user_firstname from users where user_locked=0 and access_level > 0 and user_id not in (select userid from  siprod_users)";
...
$start = $_SESSION['lvs'][$currentModule]['start'];
$navigation_array = getNavigationValues($start, $recordCount, $list_max_entries_per_page);
$navigationOutput = getTableHeaderNavigation($navigation_array, $url_string, $currentModule, 'CreateUsersGID', $viewid);
$smarty->assign("NAVIGATION", $navigationOutput);

$listview_header = getListViewHeader($focus,$currentModule,$url_string,$sorder,$order_by,'',$customView,$relatedmodule='CreateUsersGID');
//$listview_header = getListViewHeader($focus,$currentModule,$url_string,$sorder,$order_by,'',$customView);
$listview_entries = getListViewEntries($focus,$currentModule,$list_result,$navigation_array,'','','CreateUsersGID','Delete',$customView);
...
if(isset($_REQUEST['ajax']) && $_REQUEST['ajax'] != '')
	$smarty->display("CreateViewEntriesUsersGID.tpl");
else 
	$smarty->display("CreateViewUsersGID.tpl");
