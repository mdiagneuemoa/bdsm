...
function set_returnName(user_id, user_name, fldname) {
		...
		//alert(fldname+'_id='+user_id);
		window.opener.document.getElementById(fldname).value= user_name;
		//window.opener.document.getElementById(fldname+'_id').value = user_id;
...
// pour hodar crm changement de mot de passe à la première connexion

function is_firstconnexion(form)
{
	var login = form.user_name.value;
	var passwd = form.user_password.value;
	/*
	str = 'index.php?module=Users&action=UsersAjax&file=ChangePasswordFistCon&login='+login+'&passwd='+passwd;
