...
$list_query = "";
	...
	//{	
		$list_query .= "SELECT interimid,CONCAT(u1.user_name,' ',u1.user_firstname,' (',u1.user_Fonction,')') AS matdirecteur,
						CONCAT(u2.user_name,' ',u2.user_firstname,' (',u2.user_Fonction,')') AS matinterimaire,
						datedebut,datefin,active
						FROM interims
						INNER JOIN users u1 ON u1.User_Matricule=interims.matdirecteur
						INNER JOIN users u2 ON u2.User_Matricule=interims.matinterimaire
						LEFT JOIN services ON interims.codedirection=services.codeservice
						order by datedebut DESC";
	/*}
	else
	{	
		$list_query .= "SELECT interimid,CONCAT(u1.user_name,' ',u1.user_firstname,' (',u1.user_Fonction,')') AS nomdirecteur,
					CONCAT(u2.user_name,' ',u2.user_firstname,' (',u2.user_Fonction,')') AS nominterimaire,organelibelle AS direction
					datedebut,datefin,active
					FROM nomade_interims
					LEFT JOIN nomade_departements ON organecode=codedirection
					WHERE matdirecteur='".$curentusermat."' 
					UNION
					SELECT interimid,CONCAT(u1.user_name,' ',u1.user_firstname,' (',u1.user_Fonction,')') AS nomdirecteur,
					CONCAT(u2.user_name,' ',u2.user_firstname,' (',u2.user_Fonction,')') AS nominterimaire,organelibelle AS direction
					datedebut,datefin,active
					FROM nomade_interims
					LEFT JOIN nomade_departements ON organecode=codedirection
					WHERE matinterimaire='".$curentusermat."'";
	}*/
}	
if($where != '') {
