 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /cvs/repository/siprodPCCI/modules/Documents/Documents.php,v 1.1 2010/01/15 18:43:50 isene Exp $
...
class Documents extends CRMEntity {
   ...
   // var $sortby_fields = Array('title','modifiedtime','filename','createdtime','lastname','filedownloadcount','smownerid');		  
	var $sortby_fields = Array('title','modifiedtime','filename','createdtime','lastname','filedownloadcount','smownerid','publishedBy','doccategory','docstatus','potentialid');		  
	...
	var $list_fields = Array(
				...
				'published By'=>Array('notes'=>'publishedby'),
				'Document Category'=>Array('notes'=>'doccategory'),
				'Document Status'=>Array('notes'=>'docstatus'),
				'Potential'=>Array('notes'=>'potentialid')
				);
	...
	var $mandatory_fields = Array('notes_title','createdtime' ,'modifiedtime','filename','filesize','filetype','filedownloadcount','doccategory','docstatus','potentialid');
	...
	function save_module($module)
	{
		...
		//if($_REQUEST[$fieldname."_locationtype"] == 'I' ){
			if(!empty($_FILES[$fieldname]['name']) && $_FILES[$fieldname]['name']!='')
			{
				...
				$query = "Update vtiger_notes set filename = ? ,filesize = ?, filetype = ? , filelocationtype = ? , filestatus = ?, filedownloadcount = ? where notesid = ?";
			...
			{	
				...
				if ($adb->num_rows($fileres) > 0) 
				{
					...
					$query = "Update vtiger_notes set filename = ? where notesid = ?";
		...
		if($noofnotes > 0)
		{
            ...
            if($noofdeffolders == 0)
            {
			   // $insertQuery = "insert into vtiger_attachmentsfolder values (0,'Default','Contains all attachments for which a folder is not set',1,0)";
			    $insertQuery = "insert into vtiger_attachmentsfolder values (D1,'Documents','Les documents de Hodar Conseil','D1',0)";
