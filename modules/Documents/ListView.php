...
{
	BackupDocument('Archive',$_REQUEST['idToBackup']);
}
...
{
	$condition = " where folderid ='D1'";
}
$dbQuery = "select * from vtiger_attachmentsfolder ".$condition;
...
$hquery = "select * from vtiger_attachmentsfolder order by foldername asc";
...
$folder_det = getAllDocumentsFolderDetails();
...
function indent($hrarray,$folderout,$folder_det)
{
	...
	foreach($hrarray as $folderid => $value)
	{
		...
		if($folderdepth == 0){
			...
			$folderout .= '&nbsp;<b class="smallf"><a class="smallf" href="index.php?action=ListView&module=Documents&parenttab=Espace Rapportage&folderid='.$folderid.'">'.$foldername.'</a></b></td>';
			...
			$folderout .= '&nbsp;<a class="smallf" href="index.php?action=ListView&module=Documents&parenttab=Espace Rapportage&folderid='.$folderid.'">';
			...
			$folderout.='<td nowrap><div id="layer_'.$folderid.'" class="drag_Element">
						<a href="#" onClick="createFolderss(this,\'orgLay\',\''.$folderid.'\',\''.$foldername.'\',\''.$potentialid.'\',\'Document\');"><img src="' . vtiger_imageurl('Rolesadd.gif', $theme) .'" align="absmiddle" border="0" alt="Ajouter un sous-dossier" title="Ajouter un sous-dossier"></a>';
