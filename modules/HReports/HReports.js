...
function UpdateAjaxSave(label,fid,fldname,fileOrFolder)
{
	...
	if(fldVal.match(/['"\\%+?]/))
        ...
        if(fileOrFolder == 'file')
                var url='action=HReportsAjax&mode=ajax&file=Save&module=HReports&fileid='+fid+'&fldVal='+fldVal+'&fldname='+fldname+'&act=ajaxEdit';
        else
	{
                ...
                var url='action=HReportsAjax&mode=ajax&file=SaveFolder&module=HReports&record='+fid+'&foldername='+fldVal+'&savemode=Save';
	}
	if(fldname == 'status')
        {
	...
	if(fldrname.match(/[?]+$/) || fldrname.match(/[?]+/))
	{
		...
		$('status').style.display = 'block';
                new Ajax.Request(
                                ...
                                {queue: {position: 'end', scope: 'command'},
                                method: 'post',
                                postBody: 'action=HReportsAjax&mode=ajax&file=SaveFolder&module=HReports'+url,
                                onComplete: function(response) {
                                        var item = response.responseText;
					$('status').style.display = 'none';
...
function DeleteFolderCheck(folderId)
{
        ...
        new Ajax.Request(
                ...
                {queue: {position: 'end', scope: 'command'},
                        method: 'post',
                        postBody: "module=HReports&action=HReportsAjax&mode=ajax&file=DeleteFolder&deletechk=true&folderid="+folderId,
                        onComplete: function(response) {
                                        var item = response.responseText;
                                       	if(item.indexOf("NOT_PERMITTED") > -1)
					{
...
function DeleteFolder(folderId)
{
	$('status').style.display = "block";
        new Ajax.Request(
                ...
                {queue: {position: 'end', scope: 'command'},
                        method: 'post',
                        postBody: "module=HReports&action=HReportsAjax&mode=ajax&file=DeleteFolder&folderid="+folderId,
                        onComplete: function(response) {
                                        var item = response.responseText;
										$('status').style.display = "none";
                                        if(item.indexOf("FAILURE") > -1)
	...
	if(idstring != '')
	{
		if(confirm("Are you sure you want to move the file(s) to '"+foldername+"' folder ?"))
			...
			new Ajax.Request(
                        ...
                        {queue: {position: 'end', scope: 'command'},
                                method: 'post',
                                postBody: 'action=HReportsAjax&file=MoveFile&from_folderid=0&module=HReports&folderid='+id+'&idlist='+idstring,
                                onComplete: function(response) {
						var item = response.responseText;
						$('status').style.display = "none";
						if(item.indexOf("NOT_PERMITTED") > -1 )                                                                     							{
...
function dldCntIncrease(fileid)
{
	new Ajax.Request(
            ...
            {queue: {position: 'end', scope: 'command'},
             method: 'post',
             postBody: 'action=HReportsAjax&mode=ajax&file=SaveFile&module=HReports&file_id='+fileid+"&act=updateDldCnt",
             onComplete: function(response) {
                }
                }
                );
...
function checkFileIntegrity()
{
	new Ajax.Request(
            ...
            {queue: {position: 'end', scope: 'command'},
             method: 'post',
             postBody: 'module=HReports&action=HReportsAjax&mode=ajax&file=SaveFile&act=checkFileIntegrity&file_id=',
             onComplete: function(response) {
             	var item = response.responseText;
...
function checkFileIntegrityDetailView(hreportid)
{
	...
	new Ajax.Request(
            ...
            {queue: {position: 'end', scope: 'command'},
             method: 'post',
             postBody: 'module=HReports&action=HReportsAjax&mode=ajax&file=SaveFile&act=checkFileIntegrityDetailView&hreportid='+hreportid,
             onComplete: function(response) {
             	var item = response.responseText;
		...
		{
					$('vtbusy_integrity_info').style.display='none';
					$('integrity_result').innerHTML='<br><br>&nbsp;&nbsp;&nbsp;<font style=color:red>This HReports is not available. It will be marked as Inactive</font>';
					$('integrity_result').style.display='';	
					setTimeout(hideresult,6000);	
		}
                }
