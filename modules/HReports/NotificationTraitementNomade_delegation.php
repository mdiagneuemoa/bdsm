...
function getEmailChargeMission($ticket)
{
	global $log;
	$log->debug("Entering getEmailChargeMission(".$ticket.") method ...");
	$log->info("in getEmailChargeMission ".$ticket);
        ...
        if($ticket != '')
        {
			...
			FROM nomade_demande d,users agcharge,
			vtiger_crmentity vt
			WHERE vt.crmid=d.demandeid
	...
	$log->debug("Exiting getEmailChargeMission method ...");
        return $demande;
}
function getEmailSUPSInitiateur($ticket)
{
	...
	$log->info("in getEmailSUPSInitiateur ".$ticket);
        ...
        if($ticket != '')
        {
			...
			FROM nomade_demande d,vtiger_crmentity vt,nomade_hierarchie nh,users aginit
			WHERE vt.crmid=d.demandeid
			AND aginit.user_id=vt.smcreatorid
...
function getEmailInterimaire($ticket)
{
	global $log;
	$log->debug("Entering getEmailInterimaire(".$ticket.") method ...");
	$log->info("in getEmailInterimaire ".$ticket);
	...
	$log->debug("Exiting getEmailInterimaire method ...");
        return $demande;
}
function getEmailSUPSChargeMission($ticket)
{
	global $log;
	$log->debug("Entering getEmailSUPSChargeMission(".$ticket.") method ...");
	$log->info("in getEmailSUPSChargeMission ".$ticket);
	...
	$log->debug("Exiting getEmailSUPSChargeMission method ...");
        return $demande;
}
...
function getRapportMailInfo($ticket,$mode,$currentModule,$currentstatus)
	...
	$mail_data['emailschargemission'] = array('charge'=>$mailsupcharge['mailto_charge'],'chargedepart'=>$mailcharge['charge_depart'],
...
function sendNotificationAgentDepart($desc,$currentModule)
{
	...
	if ($statut == 'open') {
		$subject = $app_strings["INITIATE_MAIL_OBJECT"]." ".$desc['nom'];
		...
		if($desc['emailschargemission']['chargedepart']!=$desc['emailsinitiateur']['initiateurdepart'])
			$to_email_cc = $desc['emailsinitiateur']['initiateur'].",".$desc['emailschargemission']['directeur'].",".$desc['emailschargemission']['dircab'].",".$desc['emailschargemission']['com'];
		else
			$to_email_cc = $desc['emailsinitiateur']['initiateur'].",".$desc['emailschargemission']['directeur'];
	}
	
	if ($statut == 'dc_submitted' || $statut == 'dcpc_submitted') {
	...
	if ($statut == 'ag_cancelled' || $statut == 'om_cancelled' || $statut == 'ch_cancelled') {
...
function sendNotificationMO($desc,$currentModule)
{
	global $current_user,$adb,$mod_strings,$app_strings;
	require_once("modules/Emails/mail.php");
	...
	if ($statut == 'com_submitted') {
		$subject = $app_strings["SUBMIT_MAIL_OBJECT"]." ".$desc['nom']."  ".$app_strings["SUBMIT_MAIL_OBJECT_SUITE"];
		$to_email =$desc['emailsinitiateur']['com'];
		$to_email_cc = $desc['emailsinitiateur']['initiateur'].",".$desc['emailschargemission']['charge'];
	}
	...
	if ($statut == 'umv_accepted') {
		$subject = $app_strings["VISEUMVDL_MAIL_OBJECT"]." ".$desc['nom']."  ".$app_strings["VISEUMVDL_MAIL_OBJECT_SUITE"];
		$to_email =$desc['emailsinitiateur']['dcpc'];
		$to_email_cc = $desc['emailsinitiateur']['initiateur'].",".$desc['emailschargemission']['charge'];
	}
	
	if ($statut == 'umv_denied'  || $statut == 'dc_denied' || $statut == 'com_denied' || $statut == 'dcpc_denied') {
		$subject = $app_strings["VISEUMVDL_MAIL_OBJECT"]." ".$desc['ticket']."  ".$app_strings["REJETDL_MAIL_OBJECT_SUITE"];
		$to_email =$desc['emailschargemission']['charge'];
		$to_email_cc = $desc['emailsinitiateur']['initiateur'];
	}
	...
	if ($statut == 'ag_cancelled' || $statut == 'om_cancelled' || $statut == 'ch_cancelled' || $statut == 'om_cancelled') {
		$subject = $app_strings["CANCEL_MAIL_OBJECT"]." ".$desc['ticket']."  ".$app_strings["CANCEL_MAIL_OBJECT_SUITE"]." ".$desc['nom']."  ".$app_strings["CANCEL_MAIL_OBJECT_FIN"];
		$to_email =$desc['emailschargemission']['charge'];
		$to_email_cc=$desc['emailsinitiateur']['initiateur'];
	}
...
function sendNotificationCC($desc,$currentModule)
{
	global $current_user,$adb,$mod_strings,$app_strings;
	require_once("modules/Emails/mail.php");
	...
	if ($statut == 'submitted') {
		$subject = $app_strings["SUBMIT_MAIL_OBJECT"]." ".$desc['nom']."  ".$app_strings["SUBMIT_MAIL_OBJECT_SUITE"];
		$to_email =$desc['emailsinitiateur']['directeur'];
		$to_email_cc = $desc['emailsinitiateur']['initiateur'].",".$desc['emailschargemission']['charge'];
	}
	...
	if ($statut == 'com_accepted') {
		$subject = $app_strings["SIGN_MAIL_OBJECT"]." ".$desc['nom']."  ".$app_strings["SIGN_MAIL_OBJECT_SUITE"];
		$to_email =$desc['emailsinitiateur']['mailto_respumv'];
		$to_email_cc =  $desc['emailsinitiateur']['initiateur'].",".$desc['emailschargemission']['charge'];
	}
	...
	if ($statut == 'umv_denied'  || $statut == 'dc_denied' || $statut == 'dcpc_denied' || $statut == 'dir_denied') {
	...
	if ($statut == 'ag_cancelled' || $statut == 'umv_cancelled' || $statut == 'ch_cancelled' || $statut == 'dir_cancelled' || $statut == 'om_cancelled') {
		$subject = $app_strings["CANCEL_MAIL_OBJECT"]." ".$desc['ticket']."  ".$app_strings["CANCEL_MAIL_OBJECT_SUITE"]." ".$desc['nom']."  ".$app_strings["CANCEL_MAIL_OBJECT_FIN"];
		$to_email =$desc['emailschargemission']['charge'];
		$to_email_cc=$desc['emailsinitiateur']['initiateur'];
	}
...
function sendNotificationCJ($desc,$currentModule)
{
	global $current_user,$adb,$mod_strings,$app_strings;
	require_once("modules/Emails/mail.php");
	...
	if ($statut == 'ag_cancelled' || $statut == 'umv_cancelled' || $statut == 'ch_cancelled' || $statut == 'cancelled' || $statut == 'om_cancelled') {
		$subject = $app_strings["CANCEL_MAIL_OBJECT"]." ".$desc['ticket']."  ".$app_strings["CANCEL_MAIL_OBJECT_SUITE"]." ".$desc['nom']."  ".$app_strings["CANCEL_MAIL_OBJECT_FIN"];
		$to_email =$desc['emailschargemission']['charge'];
		$to_email_cc=$desc['emailsinitiateur']['initiateur'];
	}
...
function sendNotificationCIP($desc,$currentModule)
{
	global $current_user,$adb,$mod_strings,$app_strings;
	require_once("modules/Emails/mail.php");
	...
	if ($statut == 'umv_denied'  || $statut == 'dc_denied' || $statut == 'dcpc_denied') {
		$subject = $app_strings["REJET_MAIL_OBJECT"]." ".$desc['ticket']."  ".$app_strings["REJET_MAIL_OBJECT_FIN"];
		$to_email =$desc['emailschargemission']['charge'];
		$to_email_cc = $desc['emailsinitiateur']['initiateur'];
	}
	...
	if ($statut == 'ag_cancelled' || $statut == 'umv_cancelled' || $statut == 'ch_cancelled' || $statut == 'om_cancelled') {
		$subject = $app_strings["CANCEL_MAIL_OBJECT"]." ".$desc['ticket']."  ".$app_strings["CANCEL_MAIL_OBJECT_SUITE"]." ".$desc['nom']."  ".$app_strings["CANCEL_MAIL_OBJECT_FIN"];
		$to_email =$desc['emailschargemission']['charge'];
		$to_email_cc=$desc['emailsinitiateur']['initiateur'];
	}
							...
							<tr>
								...
								<div style="margin-bottom: 2px;"><br>';
													...
													if ($description['statut'] == 'ag_cancelled' || $description['statut'] == 'umv_tobecorrected' ||  $description['statut'] == 'dir_cancelled' || 
														...
														$description['statut'] == 'dcpc_denied' || $description['statut'] == 'open')
														{
															if ($description['motif']!='')
																$list .= '<br><b>'.$app_strings["MOTIFREJET_MAIL_TEXT"].' : </b>'.$description['motif'];
															if ($description['autremotif']!='')
																$list .= '<br><b>'.$app_strings["AUTREMOTIFREJET_MAIL_TEXT"].' : </b>'.$description['autremotif'];
															if ($description['commentaire']!='')
															$list .= '<br><b>'.$app_strings["COMMENTAIREREJET_MAIL_TEXT"].' : </b>'.$description['commentaire'];
														}	
