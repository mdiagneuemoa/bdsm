...
{	
		...
		$dbQuery = "SELECT * FROM vtiger_hreports where hreportsid= ?";
		$fileidQuery = "select attachmentsid from vtiger_seattachmentsrel where crmid = ? ";
		$result = $adb->pquery($dbQuery,array($_REQUEST['hreportid']));
		$fileidResult = $adb->pquery($fileidQuery,array($_REQUEST['hreportid']));	
		...
		$hreportsid = $adb->query_result($result,0,'hreportsid');
		...
		if(!fopen($filepath.$saved_filename, "r"))
		{
			...
			if($file_status == 1)
			{
				$dbQuery1 = "update vtiger_hreports set filestatus=0 where hreportsid= ?";
				$result1 = $adb->pquery($dbQuery1,array($hreportsid));
				echo "lost_integrity";
			}
			else 
