...
$demandeinfos = getInfosByDemande($record);
 ...
 $smarty->assign("AGENTSERVICE",$demandeinfos['service']);
  ...
  $smarty->assign("DEMANDELIEU",$demandeinfos['lieu']);
   ...
   $smarty->assign("DATEJOUR",date("Y-m-d"));
 
//print_r($demandeinfos);break;

$focus->id = $record;
//echo $focus->id,' - ',$_REQUEST['mode'];break;

if($record && $_REQUEST['mode']=='edit_view') 
{
	//$focus->id = $record;
	$focus->mode = 'edit';
...
$smarty->assign("ZONEMISSION", $ZONEMISSION);
  ...
  $smarty->assign("TIMBRES",getTimbres());	
  $smarty->assign("SIGNATAIRES",getSignataires());	  
...
if($focus->mode == 'edit') {
	...
	//echo "nextTicket=",$nextTicket;break;
	// Siprod add end
