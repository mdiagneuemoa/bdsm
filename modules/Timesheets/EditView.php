...
if($record) {
	...
	$listinterventions = $focus->getInterventions($focus->id);
	$durationHours = $focus->getDurationHours($focus->id);

	$smarty->assign('LISTINTERVENTIONS', $listinterventions);
	$smarty->assign('DURATIONHOURS', $durationHours);
}
if($isduplicate == 'true') {
	$focus->id = '';
	$focus->mode = '';
...
//$smarty->assign('LISTINTERVENTIONS', $listInterventions);
//$smarty->assign('DUREETOTALEINTERVENTIONS', $dureeTotaleIntervs);
...
// END
//$smarty->assign("DUREETOTALE",$_SESSION['addInterv']);	
//echo $_SESSION['nextID'],"--",$_SESSION['date'];

if($focus->mode == 'edit') {
	$smarty->display('salesEditViewTimesheets.tpl');
} else {
	$smarty->display('CreateViewTimesheets.tpl');
}
