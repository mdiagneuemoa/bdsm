...
if($singlepane_view == 'true' && $action == 'CallRelatedList') {
	...
	$ticket = getTicketConvention($record);
	...
	$smarty->assign("CURRENT_USER_PROFIL_ID", $current_user->profilid);	// Is Super Utilisateur if profileid = 22
	...
	$smarty->assign('TRAITEMENTS', $focus->get_traitements($ticket));
	$smarty->assign('CREXECUTIONS', $focus->get_crexecutions($ticket));
	$smarty->assign('PRODUITSFIN', $focus->get_produitsfinanciers($ticket));
	$smarty->assign('DECAISSEMENTS', $focus->get_decaissements($ticket));

	$smarty->display('RelatedListsConventions.tpl');
}
?>