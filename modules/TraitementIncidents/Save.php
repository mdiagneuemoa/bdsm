...
if( $focus->existStatut($ticket , $_REQUEST['statut']) == 1 || ( $focus->existStatut($ticket , $_REQUEST['statut']) == 0 && $mode == 'edit') ){
	...
	if($_REQUEST['statut'] == 'closed') {
		$heurefin_relle = $_REQUEST['heurefin_relle'];
		$time_start = $_REQUEST['time_start'];
		$dt = new DateTime($heurefin_relle." ".$time_start.":00");

		$date = $dt->format("Y-m-d H:i:s");
		$focus->column_fields['heurefin_relle'] = $date;

		$focus->save($currentModule);
		$return_id = $focus->id;
			
		$reqUpdate= "update siprod_incident set statut ='".$_REQUEST['statut']."', heurefin_relle= '".$date."' where ticket= '".$_REQUEST['ticket']."' " ;
		
	} else{
		$focus->save($currentModule);
		$return_id = $focus->id;
		
		$reqUpdate= "update siprod_incident set statut ='".$_REQUEST['statut']."' where ticket= '".$_REQUEST['ticket']."' " ;		
	}
...
*/
/*
if($mode == 'traited&transfered') {
	
	$_REQUEST['statut'] = 'transfered';
	$focus->column_fields['statut'] = $_REQUEST['statut'];
	
	$focus->save($currentModule);
	$return_id = $focus->id;
	$reqUpdate= "update siprod_incident set statut ='transfered' where ticket= '".$_REQUEST['ticket']."' " ;
	$adb->pquery($reqUpdate, array());
...
if( $mode == "traiter" ) {
	header("Location: index.php?module=TraitementIncidents&action=index&parenttab=Incidents");
}
elseif( $mode == "cloturer" || $mode == "reopen" ) {
	header("Location: index.php?module=SuiviIncidents&action=index&parenttab=Incidents");
}
elseif($mode == "traited&transfered") {
	$ticket = $_REQUEST['ticket'];
	header("Location: index.php?module=TraitementIncidents&opt=trsnfrt&action=EditView&dmd=$return_id&statut=transfered&ticket=$ticket&return_module=TraitementIncidents&return_action=index&parenttab=Incidents&return_viewname=");
}
