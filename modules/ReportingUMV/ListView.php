...
if($_REQUEST['filter'] == 'true') {
	list($where, $ustring) = split('#@@#', basicFilter('ReportingUMV'));
...
if ($curentuserprofil==20 ||  $curentuserprofil==26 ||  $curentuserprofil==27 ||  $curentuserprofil==30)
{
	$list_query = "SELECT DECOMPT.matricule,DECOMPT.nomcomplet,DECOMPT.codedepartement,DECOMPT.departement,CONCAT('01-01-2018',' - ','31-03-2018') AS periode,
				SUM(DECOMPT.duree) AS duree,SUM(DECOMPT.mnttotal) AS montant,COUNT(DECOMPT.omid) as nbmission,DECOMPT.datedepart,DECOMPT.datearrivee
				FROM
				(
					SELECT nomade_ordremission.omid,nomade_ordremission.numom,nomade_ordremission.matricule,CONCAT(users.user_name,'  ',users.user_firstname) AS nomcomplet,
					nomade_services.codedepartement AS codedepartement,nomade_services.libdepartement AS departement,nomade_ordremission.datedepart,nomade_ordremission.datearrivee,
					nomade_ordremission.duree,
					nomade_decompte.codebudget,nomade_decompte.lieu AS zone,
					...
					INNER JOIN nomade_ordremission ON nomade_ordremission.omid = nomade_decompte.iddemande AND nomade_ordremission.deleted=0
					INNER JOIN nomade_demande ON nomade_demande.demandeid = nomade_decompte.iddemande 
					INNER JOIN users ON  users.User_Matricule = nomade_demande.matricule
					INNER JOIN nomade_services ON  nomade_services.codeservice = users.User_direction
...
else
{
	$list_query = "SELECT DECOMPT.matricule,DECOMPT.nomcomplet,DECOMPT.codedepartement,DECOMPT.departement,CONCAT('01-01-2018',' - ','31-03-2018') AS periode,
				...
				(
					...
					FROM nomade_decompte
					INNER JOIN nomade_ordremission ON nomade_ordremission.omid = nomade_decompte.iddemande AND nomade_ordremission.deleted=0
					INNER JOIN nomade_demande ON nomade_demande.demandeid = nomade_decompte.iddemande AND  nomade_demande.matricule='".$curentusermat."' 
					INNER JOIN users ON  users.User_Matricule = nomade_demande.matricule
					INNER JOIN nomade_services ON  nomade_services.codeservice = users.User_direction
...
if($where != '') {
	...
	$list_query .=" GROUP BY DECOMPT.matricule,DECOMPT.nomcomplet,DECOMPT.departement ";
	$list_query .=" ORDER BY  DECOMPT.nomcomplet";

	$list_queryag = "select AG.matricule,AG.nomcomplet,AG.codedepartement,AG.departement,CONCAT('01-01-2018',' - ','31-03-2018') AS periode,
				SUM(AG.duree) AS duree,SUM(AG.montant) AS montant,SUM(AG.nbmission) as nbmission FROM ($list_query) AG ";
				
	//echo $list_queryag;
	$resultag = $adb->pquery($list_queryag, array());
	$rowag = $adb->fetchByAssoc($resultag);
	$smarty->assign('TOTALDUREE', $rowag['duree']);
	$smarty->assign('TOTALMONTANT', $rowag['montant']);
	$smarty->assign('TOTALMISSION', $rowag['nbmission']);
// Sorting
/*if($order_by) {
...
$smarty->assign("AGENTS", getListAgent()); 

$smarty->assign("DATEFORMAT"," jj-mm-aaaa ");
$smarty->assign("JS_DATEFORMAT", "%d-%m-%Y");
$smarty->assign("DATEDEBFILTER", "01-01-".date("Y"));
