...
{	
   ...
   $client = new SoapClient($WSDL,$SOAP_AUTH);
	...
	$mission->BENEF=$infosengagement['beneficiaire'];  // Departement bénéficiaire
	//$mission->MISSION=$infosengagement['numreunion']; // Numéro de la Réunion
	$mission->MISSION='R18-006'; // Numéro de la Réunion
	
	$mission->SOCIETE=$infosengagement['societedepart']; // Socité de la Commission ou cour des comptes
	...
	foreach($depenses as $comptenat => $depense)
	{
		...
		$mission->IT_POST->item[$i]->CODE_BUDGT=$depense['codebudget']; // Code Budgétaire
		$i++;
