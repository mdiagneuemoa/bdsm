...
$list_query = " SELECT DECAIS.numconvention ticket,convent.conventionid,convent.datesignature,convent.libellecourt libelle,convent.agenceexecution,SUM(DECAIS.montantfonds) montantconvention,
						SUM(DECAIS.montantverse) AS montantverse,SUM(DECAIS.montantfonds)-SUM(DECAIS.montantverse) AS montantrestant,
						SUM(DECAIS.montantpaye) AS paiementeffectue,SUM(DECAIS.produitsfinancier) AS produitsfinanciers,
						SUM(DECAIS.montantrestitue) AS montantrestitue,
						SUM(DECAIS.montantfonds)-SUM(DECAIS.montantpaye)+SUM(DECAIS.produitsfinancier)-SUM(DECAIS.montantrestitue) AS montantreliquatdispo,
						SUM(DECAIS.montantpaye)-SUM(DECAIS.montantfonds) AS montantprefinancement
						FROM 
						(
						SELECT c.ticket AS numconvention,c.montant AS montantfonds,
						0 montantverse,0 AS montantrestant,0 AS montantpaye,0 AS produitsfinancier,0 AS montantrestitue,0 AS montantreliquat,
						0 AS montantprefinancement
						FROM sigc_convention c
						GROUP BY  c.ticket
						UNION 
						SELECT d.numconvention,0 montantfonds,
						SUM(d.montantpaye) montantverse,0 montantrestant,0 montantpaye,
						0 produitsfinancier,0 montantrestitue,0 montantreliquat,0 montantprefinancement
						FROM sigc_decaissements d
						GROUP BY  d.numconvention
						UNION
						SELECT ex.ticket,0 montantfonds,
						0 montantverse,0 montantrestant,SUM(ex.montant) montantpaye,
						0 produitsfinancier,0 montantrestitue,0 montantreliquat,0 montantprefinancement
						FROM sigc_execution_conventions ex
						GROUP BY  ex.ticket
						UNION 
						SELECT p.ticket,0 montantfonds,
						0 montantverse,0 montantrestant,0 montantpaye,
						SUM(p.montant) produitsfinancier,0 montantrestitue,0 montantreliquat,0 montantprefinancement
						FROM sigc_produitfinancier p
						GROUP BY p.ticket
						)DECAIS
						INNER JOIN sigc_convention convent ON DECAIS.numconvention=convent.ticket ";
...
else
{
	...
	{
		$list_query .= "  AND convent.agenceexecution = '".$agence."'";

	}
	
	if($domaine!='' && $domaine!='000')
	{
		$list_query .= "  AND convent.domaine = '".$domaine."'";

	}
	if($localite!='' && $localite!='000')
	{
		$list_query .= " AND SUBSTR(convent.localite,1,2) = '".$localite."'";

	}
	if($organe!='' && $organe!='000')
	{
		$list_query .= " INNER JOIN sigc_dossiers ON sigc_dossiers.dossiercode=convent.projetid AND sigc_dossiers.organecode='".$organe."'";

	}
	
	if($programme!='' && $programme!='000')
	{
		$list_query .= " INNER JOIN sigc_dossiers ON sigc_dossiers.dossiercode=convent.projetid AND sigc_dossiers.dossierparent LIKE CONCAT('%:','".$programme."',':',sigc_dossiers.dossiercode) ";

	}
	 
	if($politique!='' && $politique!='000')
	{
		$list_query .= " INNER JOIN sigc_dossiers ON sigc_dossiers.dossiercode=convent.projetid AND sigc_dossiers.dossierparent LIKE CONCAT('".$politique."',':%:',sigc_dossiers.dossiercode) ";
	...
	$list_query .= " GROUP BY DECAIS.numconvention   ";
