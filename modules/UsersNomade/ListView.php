...
global $list_max_entries_per_page,$SEXE;
...
$list_query = "SELECT DISTINCT user_id,user_name ,user_firstname ,CONCAT(SUBSTRING(nomade_services.libservice,1,50),'...') user_direction,user_fonction ,user_email ,profilename user_profil,nomade_categorie.titrecat user_categmis
		FROM users,profile,siprod_users,categorie,services
		WHERE users.user_id=siprod_users.userid
		AND siprod_users.profilid=profile.profileid
		AND users.user_categmis=categorie.codecatuemoa
		AND users.user_direction=services.codeservice
		";

if($where != '') {
	$list_query = "$list_query AND $where";
}
$list_query .=" ORDER BY user_name,user_firstname";
// Sorting
/*if($order_by) {
