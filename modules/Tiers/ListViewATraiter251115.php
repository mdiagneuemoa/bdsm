	...
	if ($noofrecords>0){
		for($i=0;$i<$noofrecords;$i++){
			$open_indicent_list[] = array('ticket' => $adb->query_result($res,$i,'ticket'),
										'typeincident' => $adb->query_result($res,$i,'lbtypeincident'),
										'incidentid' => $adb->query_result($res,$i,'incidentid'),
	...
	if(!empty($open_indicent_list)){
		...
		foreach($open_indicent_list as $indicent){
			...
			$entries[$indicent['ticket']] = array(
					'0' => '<a href="index.php?action=DetailView&module=Incidents&record='.$indicent["incidentid"].'" ;">'.$indicent["ticket"].'',
					'1'=> $indicent["typeincident"],
					'2'=> $indicent["campagne"],
					'3'=> $indicent["popimpactee"],
					'4'=> $indicent["date_creation"],
					'5'=> $indicent["montant"],
